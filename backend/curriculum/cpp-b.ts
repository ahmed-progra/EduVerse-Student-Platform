...
export const cppB: LessonDef[] = [
  L("Dynamic Memory: new & delete", ["memory-management", "pointers"], "intermediate", 16, {
    ...
    examples: [
      ex("Allocate, use, free", `int* score = new int(42);\ncout << *score << "\\n";\n*score += 8;\ncout << *score << "\\n";\ndelete score;\nscore = nullptr;  // defuse the dangling pointer`, `42\n50`),
      ex("Runtime-sized array", `int n = 5;\nint* data = new int[n];\nfor (int i = 0; i < n; i++) data[i] = i * i;\nfor (int i = 0; i < n; i++) cout << data[i] << " ";\ncout << "\\n";\ndelete[] data;`, `0 1 4 9 16 `, `Array new pairs with array delete[] — mixing them is undefined behavior.`),
    ],
    ...
    quiz: [
      q("What is a memory leak?", ["Reading bad memory", "Heap memory never freed and no longer reachable", "Stack overflow", "Using delete twice"], 1, `The memory stays reserved until the process exits — lost capacity.`),
      q("Memory from new int[10] must be freed with…", ["delete", "delete[]", "free()", "Nothing, it's automatic"], 1, `Array allocations require the array form of delete.`),
      q("Using a pointer after deleting it is…", ["Fine if quick", "A dangling pointer — undefined behavior", "A compile error", "Automatically null"], 1, `The memory may be reused by anything; reads and writes there corrupt state.`),
    ],
  }),
  ...
  L("Structures", ["structures"], "intermediate", 12, {
    ...
    examples: [
      ex("Defining and using a struct", `struct Product {\n    string name;\n    double price;\n    int stock;\n};\n\nint main() {\n    Product p = {"Keyboard", 49.99, 12};\n    p.stock -= 1;\n    cout << p.name << " $" << p.price << " (" << p.stock << " left)\\n";\n    return 0;\n}`, `Keyboard $49.99 (11 left)`),
      ex("Structs + arrays + functions", `struct Point { double x, y; };\n\ndouble dist2(const Point& a, const Point& b) {\n    double dx = a.x - b.x, dy = a.y - b.y;\n    return dx * dx + dy * dy;\n}\n\nint main() {\n    Point path[] = {{0,0}, {3,4}};\n    cout << dist2(path[0], path[1]) << "\\n";\n    return 0;\n}`, `25`, `The arrow operator appears when you hold a pointer: ptr->x is (*ptr).x.`),
    ],
    ...
    quiz: [
      q("How do you access member x through Point* ptr?", ["ptr.x", "ptr->x", "*ptr.x", "ptr::x"], 1, `Arrow = dereference + dot. ptr->x is shorthand for (*ptr).x.`),
      q("What punctuation must follow a struct's closing brace?", ["Nothing", "A semicolon", "A comma", "A colon"], 1, `struct definitions are declarations — they end with ;`),
      q("struct vs class in C++ differs by…", ["Structs can't have methods", "Default access: struct public, class private", "Memory layout", "Speed"], 1, `That's the ONLY language difference — convention assigns them different roles.`),
    ],
  }),
  ...
  L("Classes & Encapsulation", ["classes"], "intermediate", 16, {
    ...
    quiz: [
      q("What can access a private member?", ["Any code in the file", "Only the class's own methods (and friends)", "Subclasses always", "main()"], 1, `private is enforced by the compiler — that's encapsulation's teeth.`),
      q("What does const after a method signature mean?", ["Returns a constant", "The method won't modify the object", "Compile-time only", "Static method"], 1, `const methods are callable on const objects and can't write members.`),
      q("Why use a setter instead of a public field?", ["Tradition", "The setter can validate and maintain invariants", "Performance", "Shorter code"], 1, `A gatekeeper function is the only reliable place to enforce rules on writes.`),
    ],
  }),
  ...
  L("Constructors & Destructors", ["constructors", "destructors"], "intermediate", 14, {
    ...
    examples: [
      ex("Construction and destruction order", `class Logger {\n    string name;\npublic:\n    Logger(string n) : name(n) { cout << name << " created\\n"; }\n    ~Logger() { cout << name << " destroyed\\n"; }\n};\n\nint main() {\n    Logger a("first");\n    {\n        Logger b("inner");\n    }\n    cout << "main ends\\n";\n    return 0;\n}`, `first created\ninner created\ninner destroyed\nmain ends\nfirst destroyed`, `Objects die exactly when their scope ends — deterministic, unlike garbage-collected languages.`),
      ex("Overloaded constructors", `class Timer {\n    int seconds;\npublic:\n    Timer() : seconds(60) {}\n    Timer(int s) : seconds(s) {}\n    int get() const { return seconds; }\n};\n\nint main() {\n    Timer standard;\n    Timer custom(90);\n    cout << standard.get() << " " << custom.get() << "\\n";\n    return 0;\n}`, `60 90`),
    ],
    ...
    quiz: [
      q("When does a destructor run for a local object?", ["At program exit only", "When its scope ends", "When you call delete", "Never automatically"], 1, `Stack objects are destroyed deterministically at the closing brace.`),
      q("Timer has only Timer(int s). What does 'Timer t;' do?", ["Uses defaults", "Compile error — no default constructor", "Creates t with s=0", "Runtime error"], 1, `Declaring any constructor suppresses the auto-generated default one.`),
      q("In what order are a, then b (same scope) destroyed?", ["a then b", "b then a — reverse of creation", "Simultaneously", "Unspecified"], 1, `Destruction happens in reverse declaration order.`),
    ],
  }),

  L("RAII & the Rule of Three", ["destructors", "memory-management", "advanced-cpp"], "advanced", 16, {
    ...
    examples: [
      ex("An RAII wrapper", `class IntBuffer {\n    int* data;\n    int n;\npublic:\n    IntBuffer(int size) : n(size) {\n        data = new int[n];\n        cout << "acquired " << n << " ints\\n";\n    }\n    ~IntBuffer() {\n        delete[] data;\n        cout << "released\\n";\n    }\n    int& at(int i) { return data[i]; }\n};\n\nint main() {\n    IntBuffer buf(4);\n    buf.at(0) = 99;\n    cout << buf.at(0) << "\\n";\n    return 0;\n}  // destructor frees automatically`, `acquired 4 ints\n99\nreleased`),
      ex("Why the Rule of Three exists", `// With the default copy constructor:\n// IntBuffer a(4);\n// IntBuffer b = a;   // copies the POINTER, not the array\n// ...both destructors later delete the SAME memory -> crash\n// Fix: define a deep copy, or disable copying:\n//   IntBuffer(const IntBuffer&) = delete;`, ``, `The compiler-generated copy is member-wise — for raw pointers that means double-delete disasters.`),
    ],
    ...
    quiz: [
      q("RAII ties resource release to…", ["main() ending", "The destructor of an owning object", "A cleanup() call", "The OS"], 1, `When the object dies — by any path, including exceptions — the resource frees.`),
      q("Why is the compiler-generated copy dangerous for pointer-owning classes?", ["It's slow", "It copies the pointer, so two objects delete the same memory", "It throws", "It zeroes members"], 1, `Member-wise copy shares the allocation — destruction then frees it twice.`),
      q("The Rule of Three says: if you write a destructor, also write…", ["Two more classes", "Copy constructor and copy assignment", "A getter and setter", "main()"], 1, `Resource-managing classes need all three (or explicitly deleted copies).`),
    ],
  }),
  ...
  L("Inheritance", ["inheritance"], "advanced", 14, {
    ...
    quiz: [
      q("class Dog : public Animal means…", ["Dog contains an Animal member", "Dog is-an Animal, inheriting its members", "Animal inherits Dog", "They're unrelated"], 1, `Public inheritance models the is-a relationship.`),
      q("Which access level is visible to derived classes but not outsiders?", ["private", "protected", "public", "internal"], 1, `protected sits between private and public, designed for inheritance.`),
      q("Where do you call the base constructor from a child?", ["Inside the child's body", "In the member initializer list", "You can't", "In main"], 1, `Child(...) : Base(args) — the base is constructed before the child's body runs.`),
    ],
  }),
  ...
  L("Polymorphism & Virtual Functions", ["polymorphism"], "advanced", 16, {
    ...
    examples: [
      ex("Runtime dispatch in action", `class Shape {\npublic:\n    virtual double area() const { return 0; }\n    virtual ~Shape() {}\n};\n\nclass Rect : public Shape {\n    double w, h;\npublic:\n    Rect(double w, double h) : w(w), h(h) {}\n    double area() const override { return w * h; }\n};\n\nclass Circle : public Shape {\n    double r;\npublic:\n    Circle(double r) : r(r) {}\n    double area() const override { return 3.14159 * r * r; }\n};\n\nint main() {\n    Shape* shapes[] = { new Rect(3, 4), new Circle(2) };\n    for (Shape* s : shapes) {\n        cout << s->area() << "\\n";\n        delete s;\n    }\n    return 0;\n}`, `12\n12.5664`, `One loop, one call site — each object answers with its own area(). Without virtual, both lines would print 0.`),
    ],
    ...
    quiz: [
      q("Without virtual, Animal* a = new Dog; a->speak() calls…", ["Dog::speak", "Animal::speak", "Both", "Compile error"], 1, `Non-virtual calls bind at compile time to the pointer's static type.`),
      q("What does override do?", ["Enables dispatch", "Asks the compiler to verify this really overrides a virtual base method", "Makes it faster", "Hides the base method"], 1, `It catches signature mismatches that would otherwise silently create a new method.`),
      q("Why must polymorphic bases have virtual destructors?", ["Speed", "So delete through a base pointer runs the derived destructor too", "Syntax requirement", "They don't"], 1, `Without it, the derived part is never destroyed — leaks and UB.`),
    ],
  }),
  ...
  L("Abstract Classes & Interfaces", ["polymorphism", "inheritance"], "advanced", 14, {
    ...
    quiz: [
      q("What makes a class abstract?", ["The abstract keyword", "At least one pure virtual function (= 0)", "No constructor", "Being a base class"], 1, `C++ has no abstract keyword — purity of a virtual does the job.`),
      q("What can you do with an abstract class?", ["Instantiate it", "Use pointers/references to it", "Nothing", "Only copy it"], 1, `You can't create one, but Base* and Base& are exactly how polymorphism flows.`),
      q("A child skips implementing one pure virtual. The child is…", ["Broken at runtime", "Also abstract", "Fine", "A compile error by itself"], 1, `The contract propagates until some descendant implements everything.`),
    ],
  }),
  ...
  L("Templates", ["templates"], "advanced", 16, {
    ...
    examples: [
      ex("One function, many types", `template <typename T>\nT biggest(T a, T b) {\n    return a > b ? a : b;\n}\n\nint main() {\n    cout << biggest(3, 7) << "\\n";\n    cout << biggest(2.5, 1.9) << "\\n";\n    cout << biggest(string("apple"), string("banana")) << "\\n";\n    return 0;\n}`, `7\n2.5\nbanana`),
      ex("A tiny class template", `template <typename T>\nclass Box {\n    T value;\npublic:\n    Box(T v) : value(v) {}\n    T get() const { return value; }\n};\n\nint main() {\n    Box<int> a(42);\n    Box<string> b("hello");\n    cout << a.get() << " " << b.get() << "\\n";\n    return 0;\n}`, `42 hello`, `vector<int> and map<string,int> are exactly this mechanism, scaled up.`),
    ],
    ...
    quiz: [
      q("When is template code generated for a specific type?", ["At runtime", "When the template is instantiated with that type at compile time", "When the program starts", "Never"], 1, `Each used T stamps out a concrete version during compilation.`),
      q("In biggest(3, 7), what is T?", ["double", "int — deduced from the arguments", "Unknown", "T stays generic"], 1, `Template argument deduction infers T = int.`),
      q("vector<string> is an example of…", ["Inheritance", "A class template instantiation", "A macro", "Overloading"], 1, `One vector<T> definition serves every element type.`),
    ],
  }),
  ...
  L("std::vector", ["stl"], "intermediate", 14, {
    ...
    examples: [
      ex("Growing dynamically", `vector<int> scores;\nscores.push_back(90);\nscores.push_back(85);\nscores.push_back(92);\ncout << "count: " << scores.size() << "\\n";\nfor (int s : scores) cout << s << " ";\ncout << "\\n";`, `count: 3\n90 85 92 `),
      ex("Initialize, modify, aggregate", `vector<double> prices = {19.9, 5.0, 12.5};\nprices[1] = 6.0;\ndouble total = 0;\nfor (double p : prices) total += p;\ncout << "total: " << total << "\\n";`, `total: 38.4`, `No manual memory, no size bookkeeping, no delete — the vector handles its own lifetime.`),
    ],
    ...
    quiz: [
      q("How does vector differ from a raw array?", ["It's slower always", "It grows dynamically and knows its size", "Fixed size", "No indexing"], 1, `vector manages a resizable heap array behind a safe interface.`),
      q("v[10] on a 3-element vector does what?", ["Throws", "Returns 0", "Undefined behavior", "Grows the vector"], 2, `operator[] doesn't check bounds — .at() is the checked accessor that throws.`),
      q("Which adds 7 to the end of vector v?", ["v.add(7)", "v.push_back(7)", "v.append(7)", "v += 7"], 1, `push_back is the canonical append operation.`),
    ],
  }),
  ...
  L("STL Containers: map & set", ["stl"], "advanced", 14, {
    ...
    examples: [
      ex("Counting words with a map", `map<string, int> freq;\nvector<string> words = {"to", "be", "or", "not", "to", "be"};\nfor (const string& w : words) {\n    freq[w]++;\n}\nfor (const auto& pair : freq) {\n    cout << pair.first << ": " << pair.second << "\\n";\n}`, `be: 2\nnot: 1\nor: 1\nto: 2`, `freq[w]++ works because a missing key is created as 0 — convenient here, a trap elsewhere.`),
      ex("set for uniqueness", `set<int> seen;\nint data[] = {3, 1, 4, 1, 5, 3};\nfor (int x : data) seen.insert(x);\ncout << "unique: " << seen.size() << " -> ";\nfor (int x : seen) cout << x << " ";\ncout << "\\n";`, `unique: 4 -> 1 3 4 5 `),
    ],
    ...
    quiz: [
      q("What does m[\"ghost\"] do if the key is absent?", ["Throws", "Returns null", "Inserts a default-constructed value and returns it", "Crashes"], 2, `operator[] default-inserts — the classic accidental-growth bug in lookups.`),
      q("Iterating a std::map visits keys in…", ["Insertion order", "Random order", "Sorted key order", "Reverse order"], 2, `map is a sorted (tree-based) container.`),
      q("Which container automatically rejects duplicates?", ["vector", "map values", "set", "array"], 2, `set stores each value at most once — insert of a duplicate is a no-op.`),
    ],
  }),
  ...
  L("Iterators & Range-Based For", ["stl"], "advanced", 12, {
    ...
    examples: [
      ex("Explicit iterator loop", `vector<string> names = {"Ada", "Bo", "Cy"};\nfor (auto it = names.begin(); it != names.end(); ++it) {\n    cout << *it << " ";\n}\ncout << "\\n";`, `Ada Bo Cy `),
      ex("Range-for is the same thing, nicer", `vector<int> v = {1, 2, 3};\nfor (int& x : v) {\n    x *= 10;\n}\nfor (int x : v) cout << x << " ";\ncout << "\\n";`, `10 20 30 `, `The compiler rewrites range-for into the begin/end iterator loop you just saw.`),
    ],
    ...
    quiz: [
      q("What does container.end() point to?", ["The last element", "One past the last element", "nullptr", "The first element"], 1, `end() is a sentinel boundary — never dereference it.`),
      q("for (auto& x : v) under the hood uses…", ["Indexes", "begin()/end() iterators", "Recursion", "Pointers to v itself"], 1, `Range-for is defined in terms of the iterator protocol.`),
      q("When might iterators become invalid?", ["Never", "After container modifications like push_back or erase", "After reading them", "When const"], 1, `Growth can reallocate storage; erasure shifts elements — old iterators then dangle.`),
    ],
  }),
  ...
  L("STL Algorithms", ["stl", "advanced-cpp"], "advanced", 14, {
    ...
    quiz: [
      q("sort(v.begin(), v.end(), [](int a, int b){ return a > b; }) sorts…", ["Ascending", "Descending", "Randomly", "By absolute value"], 1, `The comparator answers 'should a come before b?' — greater-than puts big first.`),
      q("Which header provides std::accumulate?", ["<algorithm>", "<numeric>", "<vector>", "<math>"], 1, `accumulate (and friends like iota) live in <numeric>.`),
      q("count_if returns…", ["The matching elements", "An iterator", "How many elements satisfy the predicate", "true/false"], 2, `It counts predicate hits across the range.`),
    ],
  }),
  ...
  L("Exception Handling", ["exception-handling"], "advanced", 14, {
    ...
    examples: [
      ex("Throwing and catching", `#include <stdexcept>\n\ndouble divide(double a, double b) {\n    if (b == 0) {\n        throw runtime_error("division by zero");\n    }\n    return a / b;\n}\n\nint main() {\n    try {\n        cout << divide(10, 2) << "\\n";\n        cout << divide(5, 0) << "\\n";\n        cout << "never reached\\n";\n    } catch (const runtime_error& e) {\n        cout << "Error: " << e.what() << "\\n";\n    }\n    cout << "program continues\\n";\n    return 0;\n}`, `5\nError: division by zero\nprogram continues`),
      ex("vector.at throws for you", `vector<int> v = {1, 2, 3};\ntry {\n    cout << v.at(10) << "\\n";\n} catch (const out_of_range& e) {\n    cout << "caught: " << e.what() << "\\n";\n}`, `caught: vector::_M_range_check: __n (which is 10) >= this->size() (which is 3)`, `at() is the bounds-checked access — [] would have been silent undefined behavior.`),
    ],
    ...
    quiz: [
      q("How should you catch exceptions?", ["By value", "By pointer", "By const reference", "Not at all"], 2, `const& avoids copying and object slicing.`),
      q("What happens to local objects between a throw and its catch?", ["Leaked", "Their destructors run during stack unwinding", "Frozen", "Copied to the handler"], 1, `Unwinding destroys locals scope by scope — the synergy with RAII.`),
      q("Which vector access throws on bad index?", ["v[i]", "v.at(i)", "v.get(i)", "Both [] and at"], 1, `at() checks bounds and throws out_of_range; [] does not check.`),
    ],
  }),
  ...
  L("Smart Pointers", ["memory-management", "advanced-cpp"], "advanced", 16, {
    ...
    examples: [
      ex("unique_ptr: automatic cleanup", `#include <memory>\n\nstruct Hero {\n    Hero() { cout << "spawned\\n"; }\n    ~Hero() { cout << "despawned\\n"; }\n    int hp = 100;\n};\n\nint main() {\n    {\n        auto hero = make_unique<Hero>();\n        hero->hp -= 30;\n        cout << "hp: " << hero->hp << "\\n";\n    }  // hero deleted here, automatically\n    cout << "after scope\\n";\n    return 0;\n}`, `spawned\nhp: 70\ndespawned\nafter scope`),
      ex("shared_ptr counts owners", `auto data = make_shared<vector<int>>();\ndata->push_back(7);\ncout << data.use_count() << "\\n";\n{\n    auto second = data;\n    cout << data.use_count() << "\\n";\n}\ncout << data.use_count() << "\\n";`, `1\n2\n1`, `The vector dies only when the LAST shared_ptr releases it.`),
    ],
    ...
    quiz: [
      q("When does unique_ptr free its object?", ["On request only", "When the unique_ptr is destroyed or reset", "At program exit", "Never"], 1, `It's RAII — the owner's destructor deletes the owned object.`),
      q("Why can't unique_ptr be copied?", ["Bug", "Copying would create two sole owners — a contradiction", "Performance", "It can"], 1, `Unique means unique; ownership moves, never duplicates.`),
      q("shared_ptr frees its object when…", ["Any owner dies", "use_count reaches zero", "After 60 seconds", "main returns"], 1, `The last owner's release triggers deletion.`),
    ],
  }),
  ...
  L("File I/O", ["io", "advanced-cpp"], "intermediate", 12, {
    ...
    examples: [
      ex("Writing a file", `#include <fstream>\n\nofstream out("scores.txt");\nif (out.is_open()) {\n    out << "Ada 95\\n";\n    out << "Bo 88\\n";\n}  // closes automatically`, ``),
      ex("Reading it back", `ifstream in("scores.txt");\nif (!in.is_open()) {\n    cout << "could not open file\\n";\n    return 1;\n}\nstring name;\nint score;\nwhile (in >> name >> score) {\n    cout << name << " scored " << score << "\\n";\n}`, `Ada scored 95\nBo scored 88`, `The while condition reads AND checks success in one step — the idiomatic stream loop.`),
    ],
    ...
    quiz: [
      q("Which class writes to files?", ["ifstream", "ofstream", "iostream", "filewriter"], 1, `o = output. ifstream reads, fstream does both.`),
      q("while (in >> x) loops until…", ["Forever", "Extraction fails (EOF or bad data)", "x is 0", "A newline"], 1, `The stream converts to false when a read fails — clean termination.`),
      q("When does an fstream close if you never call close()?", ["Never — leak", "When the stream object goes out of scope", "At a flush", "On EOF"], 1, `Streams are RAII — destruction closes the file.`),
    ],
  }),
  ...
  L("Modern C++ Practices", ["advanced-cpp"], "advanced", 14, {
    ...
    examples: [
      ex("Modern style in one snippet", `const vector<string> langs{"C++", "Python", "Rust"};\nfor (const auto& lang : langs) {\n    cout << lang << " ";\n}\ncout << "\\n";\nauto total{0};\nfor (auto i{1}; i <= 4; ++i) total += i;\ncout << total << "\\n";`, `C++ Python Rust \n10`),
      ex("Narrowing caught at compile time", `int a = 3.7;    // compiles! a == 3, silently\n// int b{3.7};  // error: narrowing conversion\ncout << a << "\\n";`, `3`, `Brace initialization turns a silent data-loss bug into a compile error.`),
    ],
    ...
    quiz: [
      q("Why prefer nullptr over NULL or 0?", ["Shorter", "It has a distinct pointer type, avoiding overload ambiguity", "Faster", "Tradition"], 1, `NULL is an integer constant — it can select the wrong overload; nullptr can't.`),
      q("int x{3.7}; does what?", ["x = 3", "x = 4", "Compile error — narrowing", "x = 3.7"], 2, `Brace initialization rejects lossy conversions at compile time.`),
      q("The modern C++ guideline for new/delete in application code is…", ["Use freely", "Avoid — use RAII types and smart pointers", "Only new is fine", "Only in classes"], 1, `Ownership belongs in types (vector, string, unique_ptr), not in manual calls.`),
    ],
  }),
];
