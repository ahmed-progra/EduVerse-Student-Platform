...
export const cssB: LessonDef[] = [
  L("Z-index & Stacking Contexts", ["positioning", "advanced-css"], "advanced", 12, {
    ...
    examples: [
      ex("The classic trap, explained", `.sidebar {\n    position: relative;\n    z-index: 1;          /* creates a stacking context */\n}\n.sidebar .dropdown {\n    position: absolute;\n    z-index: 99999;       /* trapped inside the sidebar's context! */\n}\n.header {\n    position: relative;\n    z-index: 2;           /* beats the ENTIRE sidebar bag, dropdown included */\n}`, undefined, `The dropdown's 99999 only ranks it among sidebar children. Against the header, the comparison is 1 vs 2.`),
      ex("A sane z-scale", `:root {\n    --z-dropdown: 100;\n    --z-sticky: 200;\n    --z-modal-backdrop: 900;\n    --z-modal: 1000;\n    --z-toast: 1100;\n}`),
    ],
    ...
    quiz: [
      q("z-index has no effect when the element is…", ["A flex child", "position: static (the default)", "Absolutely positioned", "In a grid"], 1, `Static elements don't participate in z ordering — position them first.`),
      q("A child with z-index 99999 renders under a sibling-of-its-parent with z-index 2. Why?", ["Browser bug", "The child is sealed inside its parent's stacking context, which ranks 1 vs 2", "Needs !important", "Wrong units"], 1, `Contexts compare as wholes; children can't escape their bag.`),
      q("Which property does NOT create a stacking context?", ["opacity: 0.9", "transform: scale(1)", "color: red", "filter: blur(2px)"], 2, `Paint-level properties like color don't; opacity/transform/filter do.`),
    ],
  }),
  ...
  L("Flexbox Fundamentals", ["flexbox"], "intermediate", 14, {
    ...
    examples: [
      ex("The navbar in five lines", `.navbar {\n    display: flex;\n    justify-content: space-between;\n    align-items: center;\n    gap: 16px;\n    padding: 12px 24px;\n}`, undefined, `Logo left, links right, everything vertically centered — the layout that used to take floats and prayers.`),
      ex("Perfect centering", `.overlay {\n    display: flex;\n    justify-content: center;\n    align-items: center;\n    min-height: 100vh;\n}`, undefined, `The two-axis centering problem that haunted CSS for 15 years: solved in three declarations.`),
    ],
    ...
    quiz: [
      q("display: flex is applied to…", ["Each item", "The container whose children you're arranging", "The body", "Both"], 1, `The parent declares the layout; children obey.`),
      q("In flex-direction: column, justify-content controls…", ["Horizontal placement", "Vertical placement (the main axis is now vertical)", "Nothing", "Text alignment"], 1, `justify always follows the main axis, wherever it points.`),
      q("The cleanest way to space flex items apart is…", ["Margins on each", "The gap property on the container", "Padding everywhere", "Empty divs"], 1, `gap handles inter-item spacing with zero edge cases.`),
    ],
  }),
  ...
  L("Flexbox: Grow, Shrink & Wrap", ["flexbox"], "intermediate", 14, {
    ...
    examples: [
      ex("The holy-grail content row", `.layout {\n    display: flex;\n    gap: 16px;\n}\n.sidebar { flex: 0 0 220px; }   /* fixed: no grow, no shrink, 220px */\n.content { flex: 1; }            /* takes ALL remaining space */`, undefined, `A fixed sidebar and fluid content — the most-built layout on the web.`),
      ex("Wrapping card rows", `.cards {\n    display: flex;\n    flex-wrap: wrap;\n    gap: 16px;\n}\n.card {\n    flex: 1 1 240px;   /* grow, shrink, ideal 240px */\n}`, undefined, `Cards sit 4-up on wide screens, 2-up on tablets, stacked on phones — zero media queries.`),
    ],
    ...
    quiz: [
      q("flex: 1 expands to…", ["grow 1 only", "grow 1, shrink 1, basis 0", "grow 1, shrink 0, basis auto", "width 100%"], 1, `That zero basis is why flex: 1 columns end up perfectly equal.`),
      q(".sidebar { flex: 0 0 220px } means…", ["Grows from 220px", "Fixed at 220px: never grows, never shrinks", "Max 220px", "Min 220px"], 1, `No grow, no shrink, basis 220 — a locked column.`),
      q("Cards with flex: 1 1 240px in a wrap container will…", ["Always be 240px", "Wrap into fewer columns as space shrinks, flexing around 240px", "Overflow", "Stack always"], 1, `Basis sets the ideal; grow/shrink and wrap negotiate the rest.`),
    ],
  }),
  ...
  L("Flexbox Patterns", ["flexbox", "modern-layout"], "intermediate", 12, {
    ...
    examples: [
      ex("Media object + auto-push", `.comment {\n    display: flex;\n    gap: 12px;\n    align-items: flex-start;\n}\n.comment img { flex: 0 0 48px; border-radius: 50%; }\n.comment .body { flex: 1; }\n.comment .time { margin-left: auto; }   /* pushed to the far right */`),
      ex("Sticky footer page", `body {\n    min-height: 100vh;\n    display: flex;\n    flex-direction: column;\n    margin: 0;\n}\nmain { flex: 1; }   /* absorbs all spare height */\nfooter { padding: 16px; }`, undefined, `Short pages: footer rests at the viewport bottom. Long pages: it follows the content. One pattern, both behaviors.`),
    ],
    ...
    quiz: [
      q("In a flex row, margin-left: auto on the last item…", ["Centers it", "Pushes it to the far right, absorbing free space", "Hides it", "Breaks layout"], 1, `Auto margins eat available space — the push-apart tool.`),
      q("The sticky-footer recipe is…", ["position: fixed footer", "body as min-100vh flex column; main gets flex: 1", "Giant margins", "A grid only"], 1, `main absorbs spare height, pinning the footer to the bottom on short pages.`),
      q("To align footers across equal-height cards…", ["Fixed heights", "Each card is a flex column; the footer has margin-top: auto", "Tables", "JavaScript"], 1, `The auto margin pushes the footer to each card's bottom edge.`),
    ],
  }),
  ...
  L("Grid Fundamentals", ["grid"], "intermediate", 14, {
    ...
    examples: [
      ex("A 3-column gallery", `.gallery {\n    display: grid;\n    grid-template-columns: repeat(3, 1fr);\n    gap: 16px;\n}`, undefined, `Nine images? Three perfect rows. No clearfixes, no width math, no wrapping bugs.`),
      ex("The responsive one-liner", `.cards {\n    display: grid;\n    grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));\n    gap: 16px;\n}`, undefined, `As many ≥220px columns as fit, all equal, at every screen size — zero media queries. The most famous line of modern CSS.`),
    ],
    ...
    quiz: [
      q("grid-template-columns: 1fr 2fr creates…", ["Two equal columns", "Two columns; the second twice as wide", "Three columns", "An error"], 1, `fr units split free space proportionally: 1 share vs 2 shares.`),
      q("What does repeat(auto-fit, minmax(220px, 1fr)) achieve?", ["Fixed 220px columns", "As many ≥220px equal columns as fit the container", "One column", "Requires media queries"], 1, `The container packs and resizes columns automatically — intrinsic responsiveness.`),
      q("Grid differs from flexbox because it…", ["Is newer", "Controls rows and columns simultaneously", "Only does columns", "Replaces HTML"], 1, `Two-dimensional track control is grid's defining power.`),
    ],
  }),
  ...
  L("Grid Template Areas", ["grid"], "intermediate", 12, {
    ...
    examples: [
      ex("The app shell, drawn", `.app {\n    display: grid;\n    grid-template-columns: 220px 1fr;\n    grid-template-rows: 56px 1fr 40px;\n    grid-template-areas:\n        "sidebar header"\n        "sidebar main"\n        "sidebar footer";\n    min-height: 100vh;\n}\n.app > header { grid-area: header; }\n.app > aside  { grid-area: sidebar; }\n.app > main   { grid-area: main; }\n.app > footer { grid-area: footer; }`, undefined, `Read the areas block — you can SEE the sidebar running down the left. Code that draws itself.`),
      ex("Mobile: redraw, don't refactor", `@media (max-width: 700px) {\n    .app {\n        grid-template-columns: 1fr;\n        grid-template-rows: 56px 1fr auto 40px;\n        grid-template-areas:\n            "header"\n            "main"\n            "sidebar"\n            "footer";\n    }\n}`, undefined, `Same HTML, totally rearranged — the sidebar drops below main on phones.`),
    ],
    ...
    quiz: [
      q("How does a child claim the \"main\" region?", ["class=\"main\"", "grid-area: main;", "area=\"main\"", "grid-cell: main"], 1, `grid-area binds the element to the named template region.`),
      q("What does \".\" mean in a template row?", ["Error", "An intentionally empty cell", "Auto content", "A 1px track"], 1, `The dot is the official empty-cell placeholder.`),
      q("Why are template areas powerful for responsive design?", ["They're faster", "Redrawing the area map re-lays-out the page without HTML changes", "They compress CSS", "They aren't"], 1, `Breakpoints swap drawings, not markup.`),
    ],
  }),
  ...
  L("Grid Placement & Spanning", ["grid", "modern-layout"], "advanced", 12, {
    ...
    examples: [
      ex("A featured card", `.grid {\n    display: grid;\n    grid-template-columns: repeat(4, 1fr);\n    gap: 16px;\n}\n.featured {\n    grid-column: span 2;\n    grid-row: span 2;\n}\n.banner {\n    grid-column: 1 / -1;   /* full width regardless of column count */\n}`, undefined, `The featured tile takes a 2×2 block; the banner always spans edge to edge.`),
      ex("Deliberate overlap", `.stack {\n    display: grid;\n}\n.stack > * {\n    grid-area: 1 / 1;   /* every child in the same cell */\n}\n.stack .caption { align-self: end; }`, undefined, `All children share one cell — image below, caption overlaid at the bottom. Grid as a layering tool.`),
    ],
    ...
    quiz: [
      q("grid-column: 1 / -1 means…", ["First column only", "From the first line to the last — full width", "Reverse order", "Invalid"], 1, `Negative indices count from the end; -1 is the final line.`),
      q("In a 4-column grid, how many vertical lines exist?", ["3", "4", "5", "8"], 2, `Lines bound the tracks: n columns → n+1 lines.`),
      q("Placing every child at grid-area: 1 / 1 creates…", ["An error", "A stack — children layer in one cell", "A single row", "Spacing"], 1, `Same-cell placement overlaps items — the modern overlay technique.`),
    ],
  }),
  ...
  L("Media Queries", ["responsive"], "intermediate", 12, {
    ...
    examples: [
      ex("Mobile-first column count", `.cards { display: grid; grid-template-columns: 1fr; gap: 16px; }\n\n@media (min-width: 600px) {\n    .cards { grid-template-columns: repeat(2, 1fr); }\n}\n@media (min-width: 1000px) {\n    .cards { grid-template-columns: repeat(4, 1fr); }\n}`, undefined, `Base styles serve phones; each query ADDS for bigger screens. One direction, no overrides fighting.`),
      ex("Respecting user preferences", `@media (prefers-color-scheme: dark) {\n    body { background: #15131f; color: #eee; }\n}\n@media (prefers-reduced-motion: reduce) {\n    * { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; }\n}`, undefined, `Dark mode and motion sensitivity are OS settings your CSS can honor automatically.`),
    ],
    ...
    quiz: [
      q("Mobile-first means…", ["Designing only for phones", "Base styles for small screens; min-width queries add for larger", "max-width queries", "An app store strategy"], 1, `You enhance upward instead of overriding downward.`),
      q("@media (prefers-reduced-motion: reduce) exists because…", ["Slow networks", "Some users get dizzy/nauseous from interface motion", "Battery saving", "Old browsers"], 1, `It's a vestibular accessibility setting your CSS should respect.`),
      q("Where should breakpoints come from?", ["Apple's device list", "Wherever YOUR layout starts to break", "Round numbers", "Frameworks only"], 1, `Content-driven breakpoints outlive any device generation.`),
    ],
  }),
  ...
  L("Fluid & Intrinsic Design", ["responsive", "modern-layout"], "advanced", 12, {
    ...
    examples: [
      ex("Fluid heading + container", `h1 {\n    font-size: clamp(1.75rem, 1.2rem + 2.5vw, 3.5rem);\n}\n.container {\n    width: min(92%, 72rem);\n    margin-inline: auto;\n}`, undefined, `The heading scales smoothly from phone to desktop — no breakpoint stairsteps. The container caps itself in one declaration.`),
      ex("The complete intrinsic card grid", `.cards {\n    display: grid;\n    grid-template-columns: repeat(auto-fit, minmax(min(100%, 240px), 1fr));\n    gap: clamp(12px, 2vw, 24px);\n}`, undefined, `min(100%, 240px) prevents overflow on ultra-narrow screens. Zero media queries, fully adaptive.`),
    ],
    ...
    quiz: [
      q("clamp(1rem, 2.5vw, 2rem) guarantees…", ["Always 2.5vw", "Never below 1rem nor above 2rem, fluid between", "Exactly 1.5rem", "Breakpoints"], 1, `clamp is a fluid value with a floor and a ceiling.`),
      q("width: min(90%, 60rem) is equivalent to…", ["width 90% only", "width: 90% with max-width: 60rem", "max() reversed", "60rem always"], 1, `min() picks the smaller — percentage on small screens, cap on large.`),
      q("Why include a rem term in fluid font formulas?", ["Tradition", "So browser/user font-size preferences still affect the text (zoom & a11y)", "Performance", "vw is buggy"], 1, `Pure-viewport text ignores user scaling — the rem component restores it.`),
    ],
  }),
  ...
  L("Transitions", ["transitions"], "intermediate", 10, {
    ...
    examples: [
      ex("A button that feels alive", `.btn {\n    background: #7c5cff;\n    transform: translateY(0);\n    box-shadow: 0 2px 6px rgb(0 0 0 / 0.15);\n    transition: transform 150ms ease-out, box-shadow 150ms ease-out;\n}\n.btn:hover {\n    transform: translateY(-2px);\n    box-shadow: 0 6px 16px rgb(0 0 0 / 0.2);\n}\n.btn:active { transform: translateY(0); }`, undefined, `150ms — interfaces should respond, not perform. The lift+shadow pair sells the depth.`),
      ex("Why transform beats top/width", `/* ✗ janky: layout recalculated every frame */\n.panel { left: -300px; transition: left 300ms; }\n\n/* ✓ smooth: GPU compositing only */\n.panel { transform: translateX(-300px); transition: transform 300ms ease-out; }`),
    ],
    ...
    quiz: [
      q("Which pair animates cheapest?", ["width & height", "transform & opacity", "top & left", "margin & padding"], 1, `They composite on the GPU without triggering layout or paint.`),
      q("Where does the transition property belong for symmetric in/out animation?", ["On :hover", "On the base state", "On both", "In a keyframe"], 1, `Declared on :hover only, the EXIT snaps with no transition.`),
      q("Why avoid transition: all?", ["Syntax error", "It animates unintended properties and costs performance", "Too fast", "Deprecated"], 1, `Explicit property lists keep motion intentional and cheap.`),
    ],
  }),
  ...
  L("Transforms", ["transitions", "animations"], "intermediate", 10, {
    ...
    examples: [
      ex("Combining transforms", `.icon:hover {\n    transform: translateY(-2px) scale(1.1) rotate(-3deg);\n    transition: transform 160ms ease-out;\n}\n.dropdown {\n    transform-origin: top center;\n    transform: scaleY(0.95);\n    opacity: 0;\n}\n.dropdown.open {\n    transform: scaleY(1);\n    opacity: 1;\n}`, undefined, `Dropdowns that grow from their origin feel attached; origin controls the story.`),
      ex("The absolute-center classic", `.modal {\n    position: fixed;\n    top: 50%;\n    left: 50%;\n    transform: translate(-50%, -50%);\n}`, undefined, `Position sets the element's top-left at center; the translate pulls it back by half its OWN size.`),
    ],
    ...
    quiz: [
      q("transform: translateX(40px) affects surrounding elements how?", ["Pushes them right", "Not at all — the element moves in its own layer", "Reflows text", "Hides them"], 1, `Transforms are visual displacement; layout stays where it was.`),
      q("The translate(-50%, -50%) centering trick works because…", ["Percentages refer to the PARENT", "Translate percentages refer to the element's OWN size", "It rounds", "Magic numbers"], 1, `That self-reference is what position offsets can't do alone.`),
      q("Does transform order matter?", ["No", "Yes — each transform operates in the coordinate space left by the previous", "Only with 3D", "Only with scale"], 1, `rotate(45deg) translateX(100px) moves diagonally; the reverse doesn't.`),
    ],
  }),
  ...
  L("Keyframe Animations", ["animations"], "advanced", 12, {
    ...
    examples: [
      ex("Spinner and entrance", `@keyframes spin {\n    to { transform: rotate(360deg); }\n}\n.loader {\n    animation: spin 0.8s linear infinite;\n}\n\n@keyframes fade-up {\n    from { opacity: 0; transform: translateY(12px); }\n    to   { opacity: 1; transform: translateY(0); }\n}\n.card {\n    animation: fade-up 0.5s ease-out both;\n}`),
      ex("Staggered list reveal", `.item { animation: fade-up 0.4s ease-out both; }\n.item:nth-child(1) { animation-delay: 0ms; }\n.item:nth-child(2) { animation-delay: 60ms; }\n.item:nth-child(3) { animation-delay: 120ms; }`, undefined, `The 60ms cascade reads as one choreographed entrance — EduVerse's own dashboard does exactly this.`),
    ],
    ...
    quiz: [
      q("Transitions vs animations?", ["Same thing", "Transitions need a property change; animations run from keyframe timelines on their own", "Animations are faster", "Transitions loop"], 1, `Keyframes give autonomous, multi-step, loopable motion.`),
      q("animation-fill-mode: both does what?", ["Plays twice", "Applies the first frame before and holds the last frame after", "Reverses", "Infinite"], 1, `Without fill, elements snap to their unanimated state at the edges.`),
      q("A loading spinner uses…", ["alternate + ease", "linear + infinite", "one iteration", "fill: none only"], 1, `Constant-speed endless rotation: linear easing, infinite count.`),
    ],
  }),
  ...
  L("CSS Variables & calc()", ["advanced-css", "modern-layout"], "advanced", 12, {
    ...
    examples: [
      ex("Tokens and a dark theme", `:root {\n    --bg: #ffffff;\n    --text: #1c1a26;\n    --accent: #7c5cff;\n    --space: 1rem;\n}\n[data-theme="dark"] {\n    --bg: #15131f;\n    --text: #eceaf4;\n    --accent: #9b82ff;\n}\nbody {\n    background: var(--bg);\n    color: var(--text);\n}\n.btn { background: var(--accent); padding: calc(var(--space) / 2) var(--space); }`, undefined, `Dark mode = one attribute flip on &lt;html&gt;. No component rewrites — the tokens carry the theme.`),
      ex("calc in layout", `.sidebar-layout main {\n    width: calc(100% - 260px);\n}\n.full-bleed {\n    margin-inline: calc(50% - 50vw);   /* escape a centered container */\n}`),
    ],
    ...
    quiz: [
      q("Where are global design tokens conventionally defined?", ["body", ":root", "html *", "@tokens"], 1, `:root has the highest-level scope — every element inherits from it.`),
      q("How does CSS-only dark mode typically work?", ["Duplicate stylesheets", "A theme selector overrides the same custom properties with dark values", "JavaScript repaints", "Filters"], 1, `Components reference tokens; themes redefine them.`),
      q("Which calc is valid?", ["calc(100%-20px)", "calc(100% - 20px)", "calc(100% -20px)", "calc(100 % - 20px)"], 1, `The +/- operators require surrounding spaces.`),
    ],
  }),
  ...
  L("Pseudo-classes & Pseudo-elements", ["selectors", "advanced-css"], "intermediate", 12, {
    ...
    examples: [
      ex("States and stripes", `tr:nth-child(even) { background: #f7f6fb; }\nbutton:focus-visible { outline: 2px solid #7c5cff; outline-offset: 2px; }\nbutton:disabled { opacity: 0.5; cursor: not-allowed; }\nli:not(:last-child) { border-bottom: 1px solid #eee; }`, undefined, `Zebra tables, visible keyboard focus, and divider-between-items — all markup-free.`),
      ex("Generated decoration", `.tag::before {\n    content: "# ";\n    color: #7c5cff;\n}\n.external::after {\n    content: " ↗";\n    font-size: 0.85em;\n}\n.quote::selection { background: #7c5cff; color: white; }`, undefined, `::before/::after are real boxes you can style, position, and animate — the decoration workhorses.`),
    ],
    ...
    quiz: [
      q("::before requires which property to render?", ["display", "content (even if empty \"\")", "position", "z-index"], 1, `No content declaration, no box — the most common pseudo-element gotcha.`),
      q(":nth-child(odd) on table rows gives you…", ["The first row", "Zebra striping", "Hover states", "Column styling"], 1, `Alternating-row backgrounds without extra classes.`),
      q(":focus-visible differs from :focus by…", ["Nothing", "Showing primarily for keyboard navigation, not mouse clicks", "Working on divs", "Being faster"], 1, `It targets focus indication where it's needed without the click-flash complaints.`),
    ],
  }),
];
