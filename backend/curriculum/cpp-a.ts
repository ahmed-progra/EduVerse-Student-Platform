...
export const cppA: LessonDef[] = [
  L("Hello, C++", ["io"], "beginner", 10, {
    ...
    examples: [
      ex("The smallest program", `#include <iostream>\nusing namespace std;\n\nint main() {\n    cout << "Hello, C++!" << endl;\n    return 0;\n}`, `Hello, C++!`),
      ex("Chaining output", `cout << "2 + 2 = " << 2 + 2 << "\\n";\ncout << "C++ since " << 1983 << "\\n";`, `2 + 2 = 4\nC++ since 1983`, `<< chains values left to right into the stream.`),
    ],
    ...
    quiz: [
      q("Where does a C++ program begin executing?", ["The first line of the file", "main()", "start()", "Anywhere"], 1, `The runtime calls main() — its body is your program.`),
      q("What does 'return 0;' in main signify?", ["Print zero", "Successful completion", "Restart", "Zero variables used"], 1, `A zero exit code conventionally means success; non-zero signals errors to the OS.`),
      q("Which header provides cout?", ["<stdio.h>", "<iostream>", "<output>", "<console>"], 1, `iostream declares the standard stream objects cout and cin.`),
    ],
  }),
  ...
  L("Variables & Types", ["variables", "data-types"], "beginner", 12, {
    ...
    examples: [
      ex("Declaring the core types", `int age = 25;\ndouble price = 19.99;\nchar grade = 'A';\nbool ready = true;\nstring name = "Ada";\ncout << name << " " << age << " " << grade << "\\n";`, `Ada 25 A`),
      ex("Types have sizes", `cout << sizeof(int) << " " << sizeof(double) << " " << sizeof(char) << "\\n";`, `4 8 1`, `Knowing data sizes is the first step toward understanding memory — C++'s superpower.`),
    ],
    ...
    quiz: [
      q("What does 'int x;' contain before assignment (local variable)?", ["0", "null", "An unpredictable garbage value", "Empty string"], 2, `Local primitives aren't auto-zeroed in C++ — reading them is undefined behavior.`),
      q("Which literal is a char?", ["\"A\"", "'A'", "`A`", "char(A)"], 1, `Single quotes make a char; double quotes make a string literal.`),
      q("C++ typing is…", ["Dynamic — types can change", "Static — fixed at compile time", "Optional", "Inferred only"], 1, `Each variable's type is fixed and checked by the compiler.`),
    ],
  }),
  ...
  L("Constants & Type Conversion", ["variables", "data-types", "operators"], "beginner", 10, {
    ...
    examples: [
      ex("Constants", `const double TAX_RATE = 0.14;\ndouble price = 200;\ncout << "Total: " << price * (1 + TAX_RATE) << "\\n";\n// TAX_RATE = 0.2;  // compile error!`, `Total: 228`),
      ex("Conversions and the division trap", `int a = 7, b = 2;\ncout << a / b << "\\n";\ncout << static_cast<double>(a) / b << "\\n";`, `3\n3.5`, `int / int is integer division. Cast one side to double to get the real quotient.`),
    ],
    ...
    quiz: [
      q("What is 9 / 4 in C++ when both are int?", ["2.25", "2", "3", "Error"], 1, `Integer division truncates toward zero.`),
      q("What happens if you assign to a const variable?", ["It changes", "Runtime error", "Compile-time error", "It becomes non-const"], 2, `The compiler rejects the program — that's the point of const.`),
      q("What does static_cast<int>(3.9) give?", ["4", "3", "3.9", "Error"], 1, `Conversion to int truncates the fraction — it does not round.`),
    ],
  }),
  ...
  L("User Input with cin", ["io"], "beginner", 10, {
    ...
    examples: [
      ex("Reading numbers", `int age;\ncout << "Age: ";\ncin >> age;\ncout << "In 10 years: " << age + 10 << "\\n";`, `Age: 20\nIn 10 years: 30`),
      ex("Words vs whole lines", `string first, full;\ncin >> first;            // stops at the first space\ncin.ignore();\ngetline(cin, full);      // reads the rest of the line\ncout << first << " | " << full << "\\n";`, `Ada Lovelace King\nAda | Lovelace King`, `>> reads a single word; getline captures spaces — mixing them needs ignore().`),
    ],
    ...
    quiz: [
      q("What does cin >> name read for input \"Grace Hopper\"?", ["Grace Hopper", "Grace", "Hopper", "Nothing"], 1, `>> extraction stops at whitespace — it reads one token.`),
      q("How do you read a full line with spaces?", ["cin.line()", "getline(cin, s)", "cin >> s >> s", "readline(s)"], 1, `getline consumes the whole line into a string.`),
      q("After cin fails on bad input, subsequent reads…", ["Work normally", "Throw exceptions", "Silently do nothing until the state is cleared", "Crash"], 2, `cin sets a fail flag; you must clear() and discard the bad input.`),
    ],
  }),
  ...
  L("Operators & Arithmetic", ["operators"], "beginner", 10, {
    ...
    quiz: [
      q("int y = x++; — what does y get?", ["x + 1", "The original x, before the increment", "Garbage", "Compile error"], 1, `Post-increment evaluates to the OLD value, then increments.`),
      q("What is 10 % 3?", ["3", "1", "0.33", "3.1"], 1, `Remainder of 10 ÷ 3 is 1.`),
      q("Which computes 2 to the 10th power?", ["2 ** 10", "2 ^ 10", "pow(2, 10)", "2.pow(10)"], 2, `C++ has no ** operator; ^ is XOR! Use pow from <cmath>.`),
    ],
  }),
  ...
  L("If / Else & Comparisons", ["conditionals"], "beginner", 10, {
    ...
    examples: [
      ex("Grade brackets", `int score = 78;\nif (score >= 90) {\n    cout << "A\\n";\n} else if (score >= 80) {\n    cout << "B\\n";\n} else if (score >= 70) {\n    cout << "C\\n";\n} else {\n    cout << "F\\n";\n}`, `C`),
      ex("Logical operators and ternary", `int age = 20;\nbool hasTicket = true;\nif (age >= 18 && hasTicket) {\n    cout << "Entry OK\\n";\n}\nstring label = (age >= 18) ? "adult" : "minor";\ncout << label << "\\n";`, `Entry OK\nadult`),
    ],
    ...
    quiz: [
      q("What does 'if (x = 0)' do?", ["Compares x to 0", "Assigns 0 to x; the condition is always false", "Compile error", "Undefined"], 1, `Assignment returns the assigned value — 0 is false, so the branch never runs (and x was clobbered).`),
      q("cond ? a : b is…", ["A loop", "An inline if/else expression", "A comparison", "Invalid C++"], 1, `The ternary operator selects between two values.`),
      q("Which is short-circuit AND?", ["&", "&&", "and only", "+"], 1, `&& stops evaluating when the left side is false. Single & is bitwise.`),
    ],
  }),
  ...
  L("Switch Statements", ["conditionals"], "beginner", 10, {
    ...
    examples: [
      ex("Menu dispatch", `int choice = 2;\nswitch (choice) {\n    case 1: cout << "New game\\n"; break;\n    case 2: cout << "Load game\\n"; break;\n    case 3: cout << "Quit\\n"; break;\n    default: cout << "Unknown option\\n";\n}`, `Load game`),
      ex("Intentional fallthrough", `char grade = 'B';\nswitch (grade) {\n    case 'A':\n    case 'B':\n        cout << "Passed with merit\\n"; break;\n    case 'C':\n        cout << "Passed\\n"; break;\n    default:\n        cout << "Try again\\n";\n}`, `Passed with merit`, `Stacking case labels with no code between them is the legitimate use of fallthrough.`),
    ],
    ...
    quiz: [
      q("What happens without break between cases?", ["Compile error", "Execution falls through into the next case", "The switch exits", "Only default runs"], 1, `Fallthrough continues into the next case's code — the #1 switch bug.`),
      q("Which type can NOT be switched on?", ["int", "char", "std::string", "enum"], 2, `switch requires integral types; strings need if/else chains.`),
      q("What is default for?", ["The first case", "Code when no case matches", "Error handling only", "Required syntax"], 1, `default is the catch-all branch, like a final else.`),
    ],
  }),
  ...
  L("While & Do-While Loops", ["loops"], "beginner", 10, {
    ...
    examples: [
      ex("Halving with while", `int n = 100;\nwhile (n > 1) {\n    n = n / 2;\n    cout << n << " ";\n}\ncout << "\\n";`, `50 25 12 6 3 1 `),
      ex("Validate with do-while", `int pin;\ndo {\n    cout << "PIN (1000-9999): ";\n    cin >> pin;\n} while (pin < 1000 || pin > 9999);\ncout << "Accepted\\n";`, `PIN (1000-9999): 99\nPIN (1000-9999): 4321\nAccepted`, `The prompt must run at least once — exactly what do-while guarantees.`),
    ],
    ...
    quiz: [
      q("How many times does a do-while body run at minimum?", ["0", "1", "2", "Depends"], 1, `The condition is tested after the first pass — one run is guaranteed.`),
      q("while (count < 5) with count starting at 10 runs…", ["5 times", "10 times", "0 times", "Forever"], 2, `The condition is false on entry, so the body is skipped entirely.`),
      q("What's syntactically required after do { } while (x > 0)?", ["Nothing", "A semicolon", "A break", "An else"], 1, `do-while is a statement and ends with ;`),
    ],
  }),
  ...
  L("For Loops", ["loops"], "beginner", 12, {
    ...
    examples: [
      ex("Counting and accumulating", `int total = 0;\nfor (int i = 1; i <= 10; i++) {\n    total += i;\n}\ncout << "Sum 1..10 = " << total << "\\n";`, `Sum 1..10 = 55`),
      ex("Nested: a triangle", `for (int row = 1; row <= 4; row++) {\n    for (int s = 0; s < row; s++) {\n        cout << "*";\n    }\n    cout << "\\n";\n}`, `*\n**\n***\n****`),
    ],
    ...
    quiz: [
      q("In for (int i = 0; i < 5; i++), how many iterations?", ["4", "5", "6", "Infinite"], 1, `i takes 0,1,2,3,4 — five passes; the loop ends when i reaches 5.`),
      q("Where does i exist after the loop ends?", ["Everywhere in main", "Nowhere — it's scoped to the loop", "Only in headers", "Global scope"], 1, `Header-declared variables die with the loop — by design.`),
      q("Which part of the header runs exactly once?", ["The condition", "The update", "The initialization", "The body"], 2, `init runs once before the first condition check.`),
    ],
  }),
  ...
  L("Arrays", ["arrays"], "beginner", 12, {
    ...
    examples: [
      ex("Declare, fill, iterate", `int scores[5] = {90, 85, 77, 92, 88};\nint sum = 0;\nfor (int i = 0; i < 5; i++) {\n    sum += scores[i];\n}\ncout << "Average: " << sum / 5.0 << "\\n";`, `Average: 86.4`),
      ex("Range-based for", `int primes[] = {2, 3, 5, 7, 11};\nfor (int p : primes) {\n    cout << p << " ";\n}\ncout << "\\n";`, `2 3 5 7 11 `, `Range-for reads cleanly when you don't need the index.`),
    ],
    ...
    quiz: [
      q("Valid indexes for int a[10] are…", ["1 to 10", "0 to 10", "0 to 9", "Any int"], 2, `Zero-based: the last element is a[9].`),
      q("What does reading a[10] on int a[10] do?", ["Returns 0", "Throws an exception", "Undefined behavior — may return garbage or crash", "Wraps to a[0]"], 2, `C++ performs no bounds checking on raw arrays.`),
      q("Can a raw C++ array grow at runtime?", ["Yes, with append", "Yes, automatically", "No — its size is fixed", "Only global ones"], 2, `Need growth? That's what std::vector is for.`),
    ],
  }),
  ...
  L("C++ Strings", ["data-types", "arrays"], "beginner", 12, {
    ...
    quiz: [
      q("What does s.substr(2, 3) return for s = \"abcdef\"?", ["\"bcd\"", "\"cde\"", "\"cd\"", "\"def\""], 1, `Start at index 2 ('c'), take 3 characters: cde.`),
      q("What does .find() return when nothing is found?", ["-1", "0", "string::npos", "An exception"], 2, `npos is the standard 'not found' sentinel — compare against it.`),
      q("Which is true about std::string vs raw char arrays?", ["Same thing", "string manages memory and supports == comparison", "char arrays are safer", "string is fixed-size"], 1, `std::string handles allocation and value comparison correctly.`),
    ],
  }),
  ...
  L("Functions", ["functions"], "intermediate", 14, {
    ...
    examples: [
      ex("Define and call", `double circleArea(double r) {\n    return 3.14159 * r * r;\n}\n\nint main() {\n    cout << circleArea(2.0) << "\\n";\n    cout << circleArea(5.0) << "\\n";\n    return 0;\n}`, `12.5664\n78.5397`),
      ex("Prototype then define", `int cube(int n);  // prototype lets main call it\n\nint main() {\n    cout << cube(4) << "\\n";\n    return 0;\n}\n\nint cube(int n) {\n    return n * n * n;\n}`, `64`, `The prototype tells the compiler the signature before the body appears.`),
    ],
    ...
    quiz: [
      q("In 'double area(int w, int h)', what is double?", ["A parameter", "The return type", "The function name", "A modifier"], 1, `C++ signatures lead with the type the function returns.`),
      q("What is a function prototype for?", ["Performance", "Declaring the signature so calls can appear before the definition", "Documentation only", "Inheritance"], 1, `The compiler needs the signature at the call site; the body can come later.`),
      q("A void function…", ["Returns 0", "Returns nothing", "Can't take parameters", "Must be empty"], 1, `void marks the absence of a return value.`),
    ],
  }),
  ...
  L("Function Overloading & Defaults", ["functions"], "intermediate", 12, {
    ...
    quiz: [
      q("Two overloads may NOT differ only by…", ["Parameter count", "Parameter types", "Return type", "Parameter order of different types"], 2, `Return type alone can't disambiguate a call site.`),
      q("When is the overload chosen?", ["At runtime", "At compile time", "Randomly", "By the linker"], 1, `Resolution is static — based on the argument types in the call.`),
      q("Default argument values must be on…", ["The first parameters", "Trailing parameters", "All parameters", "Bool parameters only"], 1, `Once a parameter has a default, all parameters after it need one too.`),
    ],
  }),
  ...
  L("Pass by Value vs Reference", ["functions", "references"], "intermediate", 14, {
    ...
    examples: [
      ex("Value vs reference, side by side", `void bumpValue(int n) { n += 10; }\nvoid bumpRef(int& n) { n += 10; }\n\nint main() {\n    int score = 50;\n    bumpValue(score);\n    cout << score << "\\n";\n    bumpRef(score);\n    cout << score << "\\n";\n    return 0;\n}`, `50\n60`),
      ex("const& for big read-only data", `int countVowels(const string& text) {\n    int c = 0;\n    for (char ch : text) {\n        if (string("aeiou").find(ch) != string::npos) c++;\n    }\n    return c;\n}\n\nint main() {\n    string essay = "references avoid copying large strings";\n    cout << countVowels(essay) << " vowels\\n";\n    return 0;\n}`, `13 vowels`, `Passing by const& means zero copying and a compiler guarantee of no modification.`),
    ],
    ...
    quiz: [
      q("void f(int n) { n = 5; } — what happens to the caller's variable?", ["Becomes 5", "Unchanged — f modified a copy", "Undefined", "Compile error"], 1, `Pass-by-value copies; the original never sees the change.`),
      q("Why pass a big string as const string&?", ["Style", "Avoids copying while preventing modification", "It's required", "Makes it global"], 1, `The reference avoids the copy; const documents and enforces read-only access.`),
      q("Which signature lets a function modify the caller's int?", ["f(int n)", "f(const int& n)", "f(int& n)", "f(int n) twice"], 2, `A non-const reference is an alias for the caller's variable.`),
    ],
  }),
  ...
  L("References", ["references"], "intermediate", 12, {
    ...
    examples: [
      ex("An alias in action", `int gold = 100;\nint& wallet = gold;\nwallet += 50;\ncout << gold << "\\n";\ngold = 0;\ncout << wallet << "\\n";`, `150\n0`),
      ex("References in range-for", `int scores[] = {70, 80, 90};\nfor (int& s : scores) {\n    s += 5;  // modifies the array elements\n}\nfor (int s : scores) cout << s << " ";\ncout << "\\n";`, `75 85 95 `, `Without & the loop variable is a copy and the array stays unchanged.`),
    ],
    ...
    quiz: [
      q("int x=1, y=2; int& r = x; r = y; — what happened?", ["r now refers to y", "x became 2", "Compile error", "y became 1"], 1, `Assignment through a reference writes to the referent; the binding to x is permanent.`),
      q("Which is TRUE of references?", ["Can be null", "Can be reseated", "Must be initialized when declared", "Take no memory ever"], 2, `A reference must bind to something at birth — no null, no reseating.`),
      q("for (auto s : strings) vs for (auto& s : strings) — the difference?", ["None", "The first copies each string; the second aliases them", "The second is invalid", "The first is faster always"], 1, `Without & every element is copied into s — costly and changes don't stick.`),
    ],
  }),
  ...
  L("Pointers", ["pointers"], "intermediate", 16, {
    ...
    examples: [
      ex("Address-of and dereference", `int treasure = 500;\nint* map = &treasure;\ncout << "address: " << map << "\\n";\ncout << "value:   " << *map << "\\n";\n*map += 100;\ncout << "treasure: " << treasure << "\\n";`, `address: 0x7ffd42a1b44c\nvalue:   500\ntreasure: 600`, `Your address will differ — it's wherever the variable landed in memory.`),
      ex("Null checks", `int* p = nullptr;\nif (p != nullptr) {\n    cout << *p << "\\n";\n} else {\n    cout << "p points at nothing\\n";\n}`, `p points at nothing`),
    ],
    ...
    template: `#include <iostream>\nusing namespace std;\n\nint main() {\n    int a = 10, b = 50;\n    int* p = &a;\n    *p += 10;\n    p = &b;\n    *p += 20;\n    cout << a << " " << b << "\\n";\n    return 0;\n}`,
    quiz: [
      q("What does *p mean when p is a pointer?", ["The address of p", "The value stored at the address p holds", "Multiplication", "A new pointer"], 1, `* dereferences — it follows the address to the value.`),
      q("What is &x?", ["The value of x", "The memory address of x", "A reference type", "x squared"], 1, `& (address-of) yields where x lives in memory.`),
      q("Dereferencing nullptr causes…", ["0", "A friendly error message", "Undefined behavior / crash", "Automatic allocation"], 2, `There's no value at address null — the program typically segfaults.`),
    ],
  }),
  ...
  L("Pointers & Arrays", ["pointers", "arrays"], "intermediate", 14, {
    ...
    examples: [
      ex("Walking an array by pointer", `int nums[] = {10, 20, 30, 40};\nint* p = nums;\ncout << *p << " " << *(p + 2) << "\\n";\nfor (int* it = nums; it != nums + 4; it++) {\n    cout << *it << " ";\n}\ncout << "\\n";`, `10 30\n10 20 30 40 `),
      ex("Arrays in functions need a size", `int sum(const int* data, int n) {\n    int total = 0;\n    for (int i = 0; i < n; i++) total += data[i];\n    return total;\n}\n\nint main() {\n    int scores[] = {5, 10, 15};\n    cout << sum(scores, 3) << "\\n";\n    return 0;\n}`, `30`, `The function receives only an address — the caller must pass the count.`),
    ],
    ...
    quiz: [
      q("If int* p points to nums[0], what is *(p + 2)?", ["nums[2]", "nums[0] + 2", "The address two bytes later", "Error"], 0, `Pointer arithmetic steps in elements: p+2 addresses index 2.`),
      q("a[i] is exactly equivalent to…", ["*(a + i)", "&a + i", "a + i", "*a + i"], 0, `Indexing is defined as dereferencing the offset address.`),
      q("Why must functions taking arrays also take a length?", ["Style", "The array decays to a pointer that carries no size", "Compilers require two args", "They don't"], 1, `Inside the function only the address remains — the count must be supplied.`),
    ],
  }),
];
