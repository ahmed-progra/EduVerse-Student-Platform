...
export const pythonA: LessonDef[] = [
  L("Hello, Python!", ["io"], "beginner", 8, {
    ...
    examples: [
      ex("A first program", `# My first program\nprint("Hello, World!")\nprint("Python is fun")`, `Hello, World!\nPython is fun`),
      ex("Printing several values at once", `print("Year:", 2026)\nprint("2 + 2 =", 2 + 2)`, `Year: 2026\n2 + 2 = 4`, `<code>print()</code> separates multiple values with a space and evaluates math before printing.`),
    ],
    ...
    quiz: [
      q("What does print(\"5\" ) output compared to print(5)?", ["They both print 5", "The first errors", "The second errors", "The first prints quotes too"], 0, `Both display 5 — but the first is text (a string) and the second is a number. The difference matters later.`),
      q("Which line is a valid comment?", ["// note", "# note", "<!-- note -->", "** note **"], 1, `Python comments start with #. The others belong to JavaScript, HTML, and Markdown.`),
      q("What happens when Python runs print(greeting) and greeting was never defined?", ["Prints empty text", "Prints the word greeting", "NameError", "SyntaxError"], 2, `Unquoted words are treated as variable names — using an undefined one raises NameError.`),
    ],
  }),
  ...
  L("Variables & Assignment", ["variables", "data-types"], "beginner", 10, {
    ...
    examples: [
      ex("Creating and using variables", `name = "Ada"\nage = 36\nprint(name, "is", age)`, `Ada is 36`),
      ex("Reassignment and swapping", `x = 1\ny = 2\nx, y = y, x\nprint(x, y)`, `2 1`, `Python can swap two variables in one line — most languages need a temporary variable.`),
    ],
    ...
    quiz: [
      q("Which is a valid Python variable name?", ["user-name", "2players", "total_score", "class"], 2, `Hyphens and leading digits are invalid, and class is a reserved keyword. snake_case with underscores is the Python convention.`),
      q("After x = 5 then x = \"five\", what is x?", ["5", "\"five\"", "Error — type can't change", "None"], 1, `Python is dynamically typed: a name can be re-bound to a value of any type.`),
      q("What does a, b = 10, 20 do?", ["Syntax error", "a=10 and b=20", "a=20 and b=10", "Creates a tuple named ab"], 1, `This is tuple unpacking — values assign left to right.`),
    ],
  }),
  ...
  L("Numbers & Math", ["data-types", "operators"], "beginner", 10, {
    ...
    examples: [
      ex("The seven operators", `a, b = 17, 5\nprint(a + b, a - b, a * b)\nprint(a / b)   # true division\nprint(a // b)  # floor division\nprint(a % b)   # remainder\nprint(a ** 2)  # power`, `22 12 85\n3.4\n3\n2\n289`),
      ex("A real calculation", `bill = 60.0\npeople = 4\ntip = bill * 0.15\nper_person = (bill + tip) / people\nprint("Each pays:", per_person)`, `Each pays: 17.25`),
    ],
    ...
    quiz: [
      q("What is 7 // 2?", ["3.5", "3", "4", "1"], 1, `Floor division drops the fractional part: 7 // 2 is 3.`),
      q("What is 2 ** 3 ** 2?", ["64", "512", "36", "Error"], 1, `** is right-associative: 3 ** 2 = 9 first, then 2 ** 9 = 512.`),
      q("Which expression checks if n is even?", ["n / 2 == 0", "n % 2 == 0", "n // 2 == 0", "n ** 2 == 0"], 1, `A number is even when dividing by 2 leaves remainder 0 — that's exactly what % tests.`),
    ],
  }),
  ...
  L("User Input", ["io", "data-types"], "beginner", 10, {
    ...
    examples: [
      ex("Reading text", `name = input("Your name: ")\nprint("Welcome,", name)`, `Your name: Ada\nWelcome, Ada`),
      ex("Reading numbers (the right way)", `age_text = input("Age: ")\nage = int(age_text)\nprint("Next year you'll be", age + 1)`, `Age: 21\nNext year you'll be 22`, `Without <code>int()</code>, <code>age + 1</code> would crash: you can't add a number to a string.`),
    ],
    ...
    quiz: [
      q("What type does input() always return?", ["int", "float", "str", "Depends on what was typed"], 2, `input() always returns a string — converting it is your job.`),
      q("The user types 7. What does input() * 2 give?", ["14", "\"77\"", "Error", "\"14\""], 1, `Multiplying a string repeats it: \"7\" * 2 is \"77\".`),
      q("What does int(\"3.9\") do?", ["Returns 3", "Returns 4", "Raises ValueError", "Returns 3.9"], 2, `int() can't parse a decimal string directly — use float(\"3.9\") first, then int() to truncate.`),
    ],
  }),
  ...
  L("Strings & Slicing", ["strings"], "beginner", 12, {
    ...
    examples: [
      ex("Indexing and length", `word = "Python"\nprint(word[0], word[-1])\nprint(len(word))`, `P n\n6`),
      ex("Slicing", `s = "programming"\nprint(s[0:3])\nprint(s[3:])\nprint(s[::-1])`, `pro\ngramming\ngnimmargorp`, `Leaving a side empty means "from the beginning" or "to the end".`),
    ],
    ...
    quiz: [
      q("For s = \"hello\", what is s[1:4]?", ["\"hel\"", "\"ell\"", "\"ello\"", "\"llo\""], 1, `Start index 1 (e) through index 3 (l) — the end index 4 is excluded.`),
      q("What does s[::-1] do?", ["Removes the first char", "Errors", "Reverses the string", "Sorts the characters"], 2, `A step of -1 walks the string backwards — the idiomatic Python reverse.`),
      q("Why does s[0] = \"H\" fail?", ["Index 0 doesn't exist", "Strings are immutable", "Wrong quote type", "It doesn't fail"], 1, `Strings can't be modified in place. Create a new string: s = \"H\" + s[1:].`),
    ],
  }),
  ...
  L("String Methods", ["strings"], "beginner", 12, {
    ...
    examples: [
      ex("Cleaning user input", `raw = "  Ada Lovelace  "\nclean = raw.strip().title()\nprint("[" + clean + "]")`, `[Ada Lovelace]`),
      ex("Split and join", `csv = "red,green,blue"\nparts = csv.split(",")\nprint(parts)\nprint(" | ".join(parts))`, `['red', 'green', 'blue']\nred | green | blue`, `split breaks a string into a list; join glues a list back into a string.`),
    ],
    ...
    quiz: [
      q("After s = \"hi\"; s.upper(), what is s?", ["\"HI\"", "\"hi\"", "\"Hi\"", "Error"], 1, `Methods return new strings — s is unchanged unless you reassign it.`),
      q("What does \"a-b-c\".split(\"-\") return?", ["\"abc\"", "['a','b','c']", "('a','b','c')", "3"], 1, `split returns a list of the pieces between separators.`),
      q("Which joins ['a','b'] into \"a+b\"?", ["['a','b'].join('+')", "'+'.join(['a','b'])", "join(['a','b'], '+')", "['a','b'] + '+'"], 1, `In Python the separator string owns the join method.`),
    ],
  }),
  ...
  L("Booleans & Comparisons", ["data-types", "operators"], "beginner", 8, {
    ...
    examples: [
      ex("Comparisons produce booleans", `score = 85\nprint(score >= 60)\nprint(score == 100)\nprint("a" < "b")  # strings compare alphabetically`, `True\nFalse\nTrue`),
      ex("Truthiness", `print(bool(""), bool("hi"))\nprint(bool(0), bool(42))\nprint(bool([]), bool([1]))`, `False True\nFalse True\nFalse True`),
    ],
    ...
    quiz: [
      q("What is the value of 3 == \"3\"?", ["True", "False", "Error", "None"], 1, `An int never equals a string in Python — no implicit conversion happens.`),
      q("Which values are falsy?", ["0, \"\", []", "1, \"a\", [0]", "Only False", "0 and False only"], 0, `Zero, empty string, and empty collections are all falsy.`),
      q("What does 1 < 2 < 3 evaluate to?", ["Error", "True", "False", "2"], 1, `Python chains comparisons: it means (1 < 2) and (2 < 3).`),
    ],
  }),
  ...
  L("If / Elif / Else", ["conditionals"], "beginner", 12, {
    ...
    examples: [
      ex("Grading", `score = 78\nif score >= 90:\n    grade = "A"\nelif score >= 80:\n    grade = "B"\nelif score >= 70:\n    grade = "C"\nelse:\n    grade = "F"\nprint(grade)`, `C`, `Order matters: the first true branch wins, so thresholds go from highest to lowest.`),
      ex("Nested decisions", `logged_in = True\nrole = "admin"\nif logged_in:\n    if role == "admin":\n        print("Dashboard: full access")\n    else:\n        print("Dashboard: read-only")\nelse:\n    print("Please log in")`, `Dashboard: full access`),
    ],
    ...
    template: `age = 12\nif age < 5:\n    price = 0\nelif age <= 17:\n    price = 8\nelif age <= 64:\n    price = 15\nelse:\n    price = 10\nprint("Ticket:", price)`,
    quiz: [
      q("In an if/elif/elif/else chain, how many blocks can run?", ["All true ones", "Exactly one", "At most one... else guarantees one", "Two"], 2, `Exactly one block runs — the first true condition, or else as fallback. With else present, exactly one always runs.`),
      q("score = 95 with 'if score >= 70: C' checked before 'elif score >= 90: A' prints…", ["A", "C", "Error", "Nothing"], 1, `The first true branch wins — 95 >= 70 matches immediately. Threshold order is a classic bug.`),
      q("What defines a block's boundaries in Python?", ["Curly braces", "Parentheses", "Indentation", "Semicolons"], 2, `Python uses consistent indentation instead of braces.`),
    ],
  }),
  ...
  L("Logical Operators", ["conditionals", "operators"], "beginner", 10, {
    ...
    examples: [
      ex("Combining conditions", `age = 25\nhas_id = True\nif age >= 18 and has_id:\n    print("Entry allowed")\nif age < 13 or age > 65:\n    print("Discount applies")\nelse:\n    print("Full price")`, `Entry allowed\nFull price`),
      ex("Short-circuit protects against errors", `items = []\nif len(items) > 0 and items[0] == "a":\n    print("starts with a")\nelse:\n    print("empty or different")`, `empty or different`, `Because the left side is False, Python never evaluates <code>items[0]</code> — avoiding an IndexError.`),
    ],
    ...
    quiz: [
      q("What is True or sketchy_function() — does the function run?", ["Yes, always", "No — or short-circuits", "Only if it returns True", "Syntax error"], 1, `or stops at the first truthy value; the left True decides the answer immediately.`),
      q("Why is 'if x == 1 or 2:' a bug?", ["Syntax error", "2 is always truthy so the condition is always true", "or can't follow ==", "It compares x to 3"], 1, `It parses as (x == 1) or (2). The literal 2 is truthy, so the branch always runs.`),
      q("not (a or b) is equivalent to…", ["not a or not b", "not a and not b", "a and b", "not a or b"], 1, `De Morgan's law: negation distributes by flipping or into and.`),
    ],
  }),
  ...
  L("While Loops", ["loops"], "beginner", 10, {
    ...
    examples: [
      ex("Counting", `count = 1\nwhile count <= 5:\n    print("Round", count)\n    count += 1\nprint("Done")`, `Round 1\nRound 2\nRound 3\nRound 4\nRound 5\nDone`),
      ex("Repeat until valid (sentinel)", `pin = ""\nwhile pin != "1234":\n    pin = input("Enter PIN: ")\nprint("Unlocked")`, `Enter PIN: 0000\nEnter PIN: 1234\nUnlocked`),
    ],
    ...
    template: `n = 100\nsteps = 0\nwhile n > 0:\n    n = n // 2\n    steps += 1\n    print(n)\nprint("Steps:", steps)`,
    quiz: [
      q("When is a while condition checked?", ["After each iteration", "Before each iteration", "Only once at the start", "Only when break runs"], 1, `The condition gates entry to every iteration — if it's false initially, the body never runs.`),
      q("What's wrong with: while x < 10: print(x)?", ["Syntax error", "x is checked too often", "x never changes — infinite loop", "print can't be in a while"], 2, `Nothing in the body moves x toward 10, so the condition stays true forever.`),
      q("count = 0; while count < 3: count += 1 — how many iterations?", ["2", "3", "4", "Infinite"], 1, `count goes 0→1→2→3; the check fails when count reaches 3. Three iterations.`),
    ],
  }),
  ...
  L("For Loops & range()", ["loops"], "beginner", 12, {
    ...
    examples: [
      ex("Looping over items and numbers", `for ch in "abc":\n    print(ch)\nfor i in range(3):\n    print("i =", i)`, `a\nb\nc\ni = 0\ni = 1\ni = 2`),
      ex("Accumulator: summing", `total = 0\nfor n in range(1, 101):\n    total += n\nprint(total)`, `5050`, `Gauss's classic — summing 1 to 100. range(1, 101) includes 1, excludes 101.`),
    ],
    ...
    quiz: [
      q("What numbers does range(2, 10, 3) produce?", ["2,5,8", "2,5,8,10", "3,6,9", "2,4,6,8"], 0, `Start at 2, step by 3, stop before 10: 2, 5, 8.`),
      q("How do you loop from 10 down to 1?", ["range(10, 1)", "range(10, 0, -1)", "range(1, 10, -1)", "reverse(range(10))"], 1, `A negative step counts down; stop is exclusive so 0 ends the loop at 1.`),
      q("What is the loop variable in 'for ch in \"hi\":' on the second iteration?", ["\"h\"", "\"i\"", "1", "\"hi\""], 1, `The variable takes each character in order: 'h' then 'i'.`),
    ],
  }),
  ...
  L("Loop Control: break & continue", ["loops"], "beginner", 10, {
    ...
    examples: [
      ex("Finding the first match", `nums = [7, 11, 4, 9, 2]\nfor n in nums:\n    if n % 2 == 0:\n        print("First even:", n)\n        break\nelse:\n    print("No even number found")`, `First even: 4`, `The else clause would run only if no break happened — a uniquely Python feature.`),
      ex("Skipping with continue", `for n in range(1, 8):\n    if n % 2 == 0:\n        continue\n    print(n)`, `1\n3\n5\n7`),
    ],
    ...
    quiz: [
      q("What does continue do?", ["Exits the loop", "Skips to the next iteration", "Restarts the loop from zero", "Pauses the loop"], 1, `continue abandons the current iteration only; the loop itself keeps going.`),
      q("In nested loops, break exits…", ["All loops", "The innermost loop only", "The outermost loop only", "The function"], 1, `break affects only the nearest enclosing loop.`),
      q("When does a for loop's else clause run?", ["Always", "Never", "Only if the loop completed without break", "Only after break"], 2, `loop-else is 'no break happened' — handy for search-failed cases.`),
    ],
  }),
  ...
  L("Lists", ["lists"], "beginner", 12, {
    ...
    examples: [
      ex("Building a list", `tasks = []\ntasks.append("write code")\ntasks.append("test code")\ntasks.insert(0, "plan")\nprint(tasks)\nprint(len(tasks))`, `['plan', 'write code', 'test code']\n3`),
      ex("Changing and removing", `nums = [10, 20, 30, 40]\nnums[1] = 99\nlast = nums.pop()\nnums.remove(10)\nprint(nums, "popped:", last)`, `[99, 30] popped: 40`, `pop() returns the removed item; remove() searches by value.`),
    ],
    ...
    quiz: [
      q("After a = [1,2,3]; b = a; b.append(4), what is a?", ["[1,2,3]", "[1,2,3,4]", "Error", "[4]"], 1, `b = a copies the reference, not the list — both names see the same object.`),
      q("Which removes and returns the last item?", [".remove(-1)", ".pop()", ".delete()", ".cut()"], 1, `pop() with no argument takes from the end and hands the item back.`),
      q("What does [1, 2] + [3] produce?", ["[1,2,3]", "[1,5]", "Error", "[4, 3]"], 0, `+ concatenates lists into a new list.`),
    ],
  }),
  ...
  L("List Methods & Sorting", ["lists"], "beginner", 12, {
    ...
    examples: [
      ex("sort vs sorted", `nums = [3, 1, 4, 1, 5]\nfresh = sorted(nums)\nprint(nums, fresh)\nnums.sort(reverse=True)\nprint(nums)`, `[3, 1, 4, 1, 5] [1, 1, 3, 4, 5]\n[5, 4, 3, 1, 1]`),
      ex("Sorting by a key", `words = ["banana", "fig", "cherry"]\nwords.sort(key=len)\nprint(words)`, `['fig', 'banana', 'cherry']`, `key tells sort what to compare — here, each word's length.`),
    ],
    ...
    quiz: [
      q("What does x = [3,1,2].sort() assign to x?", ["[1,2,3]", "None", "[3,1,2]", "Error"], 1, `.sort() mutates the list and returns None — a classic Python gotcha.`),
      q("How do you sort words by length, longest first?", ["words.sort(len, reverse)", "sorted(words, key=len, reverse=True)", "words.sort().reverse()", "sorted(words, by=len)"], 1, `key picks the comparison value; reverse=True flips the order.`),
      q("What is [5,2,8,1][1:3] after sorting the list ascending?", ["[2,5]", "[2,8]", "[1,2]", "[5,8]"], 0, `Sorted: [1,2,5,8]; slice 1:3 takes indexes 1 and 2 → [2,5].`),
    ],
  }),
  ...
  L("Nested Loops & 2D Data", ["loops", "lists"], "intermediate", 14, {
    ...
    examples: [
      ex("Walking a grid", `grid = [[1, 2, 3], [4, 5, 6]]\nfor row in grid:\n    for cell in row:\n        print(cell, end=" ")\n    print()`, `1 2 3 \n4 5 6 `),
      ex("All combinations", `sizes = ["S", "M"]\ncolors = ["red", "blue"]\nfor s in sizes:\n    for c in colors:\n        print(s, c)`, `S red\nS blue\nM red\nM blue`),
    ],
    ...
    quiz: [
      q("How many times does the inner body run for 'for i in range(3): for j in range(4):'?", ["7", "12", "3", "4"], 1, `The inner loop runs fully for each outer pass: 3 × 4 = 12.`),
      q("In grid = [[1,2],[3,4]], what is grid[1][0]?", ["1", "2", "3", "4"], 2, `Row index 1 is [3,4]; column 0 of that row is 3.`),
      q("What prints S1 S2 M1 M2?", ["Two separate loops", "for s in 'SM': for n in '12': print(s+n)", "while loops only", "A single range"], 1, `Nested loops generate every combination of the outer and inner sequences.`),
    ],
  }),
  ...
  L("Tuples", ["tuples"], "intermediate", 10, {
    ...
    examples: [
      ex("Tuples and unpacking", `point = (3, 7)\nx, y = point\nprint("x =", x, "y =", y)\nprint(point[0] * point[1])`, `x = 3 y = 7\n21`),
      ex("Pairs in a loop", `entries = [("Ada", 95), ("Linus", 88)]\nfor name, score in entries:\n    print(name, "scored", score)`, `Ada scored 95\nLinus scored 88`, `Unpacking right inside the for statement is idiomatic Python.`),
    ],
    ...
    quiz: [
      q("How do you write a one-element tuple containing 7?", ["(7)", "(7,)", "tuple(7)", "[7]"], 1, `Without the trailing comma, (7) is just a parenthesized number.`),
      q("What does x, y = (10, 20) do?", ["Error", "x=10, y=20", "x=(10,20), y undefined", "x=20, y=10"], 1, `Tuple unpacking assigns positionally.`),
      q("Why choose a tuple over a list?", ["Faster appends", "It signals fixed, unchangeable structure", "Tuples can hold mixed types, lists can't", "No reason"], 1, `Immutability communicates intent and lets tuples serve as dict keys — lists can't.`),
    ],
  }),
  ...
  L("Sets", ["sets"], "intermediate", 10, {
    ...
    examples: [
      ex("Dedup in one line", `votes = ["ana", "bo", "ana", "cy", "bo"]\nunique = set(votes)\nprint(unique)\nprint(len(unique), "distinct voters")`, `{'cy', 'ana', 'bo'}\n3 distinct voters`, `Order isn't guaranteed — sets are about membership, not position.`),
      ex("Set algebra", `py_devs = {"ana", "bo", "cy"}\njs_devs = {"bo", "cy", "dee"}\nprint(py_devs & js_devs)\nprint(py_devs | js_devs)\nprint(py_devs - js_devs)`, `{'bo', 'cy'}\n{'ana', 'bo', 'cy', 'dee'}\n{'ana'}`),
    ],
    ...
    quiz: [
      q("What is len({1, 2, 2, 3, 3, 3})?", ["6", "3", "1", "Error"], 1, `Duplicates collapse on creation — the set holds {1, 2, 3}.`),
      q("How do you create an empty set?", ["{}", "set()", "[]", "( )"], 1, `{} is an empty dict; set() is the only way to make an empty set.`),
      q("Given a = {1,2,3}, b = {2,3,4}: what is a - b?", ["{1}", "{4}", "{2,3}", "{1,4}"], 0, `Difference keeps what's in a but not in b.`),
    ],
  }),
  ...
  L("Dictionaries", ["dictionaries"], "intermediate", 14, {
    ...
    examples: [
      ex("A record", `user = {"name": "Ada", "age": 36}\nprint(user["name"])\nuser["age"] += 1\nuser["title"] = "Countess"\nprint(user)`, `Ada\n{'name': 'Ada', 'age': 37, 'title': 'Countess'}`),
      ex("Counting with a dict", `text = "to be or not to be"\ncounts = {}\nfor word in text.split():\n    counts[word] = counts.get(word, 0) + 1\nprint(counts)`, `{'to': 2, 'be': 2, 'or': 1, 'not': 1}`, `get(word, 0) returns 0 for unseen words — the canonical counting idiom.`),
    ],
    ...
    quiz: [
      q("What does d.get(\"x\", 0) return when \"x\" isn't in d?", ["KeyError", "None always", "0", "\"x\""], 2, `get takes a default — returned instead of raising KeyError.`),
      q("Which can be a dict key?", ["[1, 2]", "{\"a\": 1}", "(\"a\", 1)", "set()"], 2, `Keys must be immutable — tuples qualify; lists, dicts, and sets don't.`),
      q("How do you loop over keys AND values together?", ["for k, v in d:", "for k, v in d.items():", "for kv in d.all():", "for k in d.values():"], 1, `.items() yields (key, value) tuples ready for unpacking.`),
    ],
  }),
];
