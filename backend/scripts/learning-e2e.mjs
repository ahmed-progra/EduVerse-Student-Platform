...
const CREDS = {
  email: `learn-e2e-${STAMP}@eduverse.dev`,
  username: `learn_${STAMP}`,
  password: "test12345",
};
...
async function main() {
  ...
  check(
    "setup: python course exists with expanded curriculum",
    !!python && python.lessons.length >= 36,
  ...
  check(
    "state: not assessed before placement",
    res.status === 200 && res.data?.data?.assessed === false,
    `questions=${res.data?.data?.questionCount}`,
  );

  // ── Negative: submit without starting ──
  res = await call(`/learning/${courseId}/assessment/submit`, {
    method: "POST",
    body: { assessmentId: "nope", answers: {} },
  });
  ...
  check(
    "start: returns questions + session id",
    res.status === 200 && !!assessmentId && questions.length >= 20,
    `${questions.length} questions`,
  );
  check(
    "start: answers are NOT leaked to the client",
    questions.every((q) => q.answer === undefined),
  );
  check(
    "start: includes mcq, predict, and code types",
    ["mcq", "predict", "code"].every((t) => questions.some((q) => q.type === t)),
  );
  ...
  check(
    "submit: succeeds with level classification",
    res.status === 200 && ["beginner", "intermediate", "advanced"].includes(sub?.level),
    `level=${sub?.level} score=${sub?.scorePct}%`,
  );
  check(
    "submit: topic mastery covers all 21 Python topics",
    sub && Object.keys(sub.mastery || {}).length === 21,
  );
  check(
    "submit: mastered + missing topics identified",
    Array.isArray(sub?.masteredTopics) &&
      sub.masteredTopics.length > 0 &&
      Array.isArray(sub?.missingTopics) &&
      sub.missingTopics.length > 0,
    `mastered=${sub?.masteredTopics?.length} gaps=${sub?.missingTopics?.length}`,
  );
  check(
    "submit: AI summary present",
    typeof sub?.summary === "string" && sub.summary.length > 30,
    `"${(sub?.summary || "").slice(0, 70)}..."`,
  );
  check(
    "submit: strengths + weaknesses listed",
    (sub?.strengths?.length || 0) > 0 && (sub?.weaknesses?.length || 0) > 0,
  );
  check(
    "submit: code task got AI feedback",
    typeof sub?.codeFeedback?.["py-code1"] === "string" && sub.codeFeedback["py-code1"].length > 5,
    `"${(sub?.codeFeedback?.["py-code1"] || "").slice(0, 60)}..."`,
  );
  ...
  check(
    "roadmap: generated with required + skipped lessons",
    required.length > 0,
    `${required.length} required, ${skipped.length} skipped`,
  );
  check(
    "roadmap: intermediate+ learner skips mastered lessons",
    sub?.level === "beginner" ? skipped.length === 0 : skipped.length > 0,
  );
  check(
    "roadmap: EVERY skipped lesson has a justification",
    skipped.every((i) => typeof i.reason === "string" && i.reason.length > 10),
    skipped[0] ? `"${skipped[0].reason.slice(0, 60)}..."` : "",
  );
  check(
    "roadmap: estimated completion time computed",
    (roadmap?.estMinutes || 0) > 0,
  ...
  check(
    "state: assessed with persisted profile + roadmap",
    st?.assessed === true && !!st?.profile && !!st?.roadmap,
    `profile level=${st?.profile?.level} v${st?.roadmap?.version}`,
  );
  ...
  check(
    "lesson: serves quiz without answers",
    Array.isArray(lesson?.quiz) &&
      lesson.quiz.length >= 2 &&
      lesson.quiz.every((q) => q.answer === undefined),
    `${lesson?.quiz?.length} questions`,
  );

  // First attempt: all zeros (probably imperfect)
  res = await call(`/lessons/${firstRequired.lessonId}/quiz`, {
    method: "POST",
    body: { answers: lesson.quiz.map(() => 0) },
  });
  const quiz1 = res.data?.data;
  check(
    "quiz: graded server-side with explanations",
    res.status === 200 &&
      Array.isArray(quiz1?.results) &&
      quiz1.results.every((r) => typeof r.explain === "string"),
    `${quiz1?.correct}/${quiz1?.total}`,
  );

  // Second attempt: use revealed answers → must pass
  res = await call(`/lessons/${firstRequired.lessonId}/quiz`, {
    method: "POST",
    body: { answers: quiz1.results.map((r) => r.answer) },
  });
  const quiz2 = res.data?.data;
  check(
    "quiz: perfect retake passes with XP",
    quiz2?.passed === true && quiz2.pct === 100 && quiz2.xpGained > 0,
    `+${quiz2?.xpGained} XP`,
  );
  ...
  check(
    "adaptation: roadmap version advanced after quiz events",
    (st2?.roadmap?.version || 0) > versionBefore,
    `v${versionBefore} → v${st2?.roadmap?.version}`,
  );

  // ── Lesson completion feeds adaptation too ──
  res = await call(`/lessons/${firstRequired.lessonId}/complete`, { method: "POST", body: {} });
  check(
    "complete: lesson completion awards XP",
    res.status === 200 && (res.data?.data?.xpGained || 0) > 0,
    `+${res.data?.data?.xpGained} XP`,
  );
  res = await call(`/learning/${courseId}/state`);
  const st3 = res.data?.data;
  const completedInRoadmap = (st3?.roadmap?.items || []).find(
    (i) => i.lessonId === firstRequired.lessonId,
  )?.completed;
  ...
  check(
    "refresh: AI re-analysis returns updated plan",
    res.status === 200 &&
      ["beginner", "intermediate", "advanced"].includes(ref?.level) &&
      (ref?.roadmap?.items?.length || 0) > 0,
    `level=${ref?.level} "${(ref?.summary || "").slice(0, 50)}..."`,
  );
  ...
  check(
    "multi-course: HTML has its own independent gate",
    res.status === 200 && res.data?.data?.assessed === false && res.data?.data?.questionCount > 0,
    `${res.data?.data?.questionCount} questions`,
  );
