...
const CREDS = { email: `appr-e2e-${STAMP}@eduverse.dev`, username: `appr_${STAMP}`, password: "test12345" };
...
async function main() {
  ...
  check("topics: catalog returns courses with topics", res.status === 200 && courses.length > 0 && (courses[0].topics?.length || 0) > 0, `${courses.length} courses, maxTurns=${maxTurns}`);
  ...
  res = await call("/apprentice/grade", { method: "POST", body: { topic: "Loops", turns: [{ role: "apprentice", text: "hi" }] } });
  check("guard: grading requires the mentor to have taught something", res.status === 400);

  // ── Start a session ──
  console.log("  ... Pip is meeting you (live AI)");
  res = await call("/apprentice/start", { method: "POST", body: { topic: loops.label, courseLabel: "Python" } });
  const opener = res.data?.data;
  check("start: Pip greets and asks a question", res.status === 200 && typeof opener?.say === "string" && opener.say.length > 15, `"${(opener?.say || "").slice(0, 60)}..."`);
  check("start: understanding begins low", typeof opener?.understanding === "number" && opener.understanding <= 40, `understanding=${opener?.understanding}`);
  ...
  for (let i = 0; i < LESSON.length; i++) {
    ...
    res = await call("/apprentice/reply", { method: "POST", body: { topic: loops.label, turns, turnIndex: i } });
    const reply = res.data?.data;
    check(`reply ${i + 1}: Pip responds in character`, res.status === 200 && typeof reply?.say === "string" && reply.say.length > 10, `understanding=${reply?.understanding}`);
    turns.push({ role: "apprentice", text: reply.say });
    lastUnderstanding = reply.understanding ?? lastUnderstanding;
    finalDone = reply.done;
  }
  check("teaching: Pip's understanding rose as you taught", lastUnderstanding > (opener.understanding ?? 0), `${opener.understanding} → ${lastUnderstanding}`);

  // ── Grade ──
  console.log("  ... grading your teaching (live AI)");
  res = await call("/apprentice/grade", { method: "POST", body: { topic: loops.label, topicKey: loops.key, courseSlug: "python", turns } });
  const grade = res.data?.data;
  check("grade: returns clarity/correctness/completeness/overall", res.status === 200 && ["clarity", "correctness", "completeness", "overall"].every((k) => typeof grade?.[k] === "number"), `overall=${grade?.overall}`);
  check("grade: verdict + strengths + improvements present", typeof grade?.verdict === "string" && grade.verdict.length > 10 && Array.isArray(grade?.strengths) && Array.isArray(grade?.improvements), `"${(grade?.verdict || "").slice(0, 50)}..."`);
  check("grade: XP awarded for teaching", (grade?.xpAwarded || 0) > 0, `+${grade?.xpAwarded} XP`);
  check("grade: masteryBoosted flag present (false without a prior assessment)", typeof grade?.masteryBoosted === "boolean", `masteryBoosted=${grade?.masteryBoosted}`);

  // ── XP reflected on the account ──
  res = await call("/auth/me");
  const xpAfter = res.data?.data?.xp ?? 0;
  check("xp: teaching XP credited to the learner", xpAfter > xpBefore, `${xpBefore} → ${xpAfter} XP`);
