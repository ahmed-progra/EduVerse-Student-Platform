  ...
  return (
    <div className="min-h-screen">
      ...
      <header
        ...
        style={{ borderColor: "var(--color-eduverse-border)" }}
      >
        <Link
          href="/"
          className="text-xl font-bold"
          style={{ fontFamily: "var(--font-display)", color: "var(--color-eduverse-text)" }}
        >
          EduVerse
        </Link>
      ...
      <main className="max-w-4xl mx-auto px-5 md:px-8 py-10">
        {state === "loading" && (
          <div className="space-y-6" aria-hidden="true">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="sk-card" style={{ height: "160px" }} />
              ))}
            </div>
          </div>
        )}
        ...
        {state === "notfound" && (
          <div className="text-center py-24">
            <Rocket
              className="w-10 h-10 text-eduverse-text-muted mx-auto mb-4"
              aria-hidden="true"
            />
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            ...
            <motion.div variants={fadeUp} transition={fastEaseTransition}>
              <div className="app-card p-6 flex items-center gap-5 flex-wrap">
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold font-mono shrink-0"
                  style={{
                ...
                <div className="min-w-0">
                  <h1 className="text-2xl font-bold font-display text-eduverse-text">
                    {data.user.username}
                  </h1>
                  <div className="flex items-center gap-3 mt-1 text-sm text-eduverse-text-muted flex-wrap">
                    <span className="flex items-center gap-1 capitalize">
                      <Gauge size={14} className="text-eduverse-accent" aria-hidden="true" />{" "}
                      {data.user.placementLevel}
                    </span>
                    <span className="flex items-center gap-1">
                      <Sparkles size={14} className="text-eduverse-accent" aria-hidden="true" />{" "}
                      Level {data.user.level}
                    </span>
                    <span className="flex items-center gap-1">
                      <Trophy size={14} className="text-eduverse-accent" aria-hidden="true" />{" "}
                      {data.projects.length} {data.projects.length === 1 ? "project" : "projects"}
                    </span>
                    <span className="flex items-center gap-1">
                      <Calendar size={14} aria-hidden="true" /> Since{" "}
                  ...
                  {data.user.bio && (
                    <p className="text-sm text-eduverse-text-body mt-2">{data.user.bio}</p>
                  )}
                </div>
              </div>
            </motion.div>
            ...
            {data.projects.length === 0 ? (
              <motion.div variants={fadeUp} transition={fastEaseTransition}>
                <div className="app-card p-10 text-center">
                  <Rocket
                    className="w-8 h-8 text-eduverse-text-muted mx-auto mb-3"
                    aria-hidden="true"
                  />
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {data.projects.map((p, i) => (
                  <motion.div
                    key={p.id}
                    variants={fadeUp}
                    transition={{ ...fastEaseTransition, delay: 0.05 + i * 0.05 }}
                  >
                    <div className="app-card p-5 h-full">
                      ...
                      <p className="text-xs text-eduverse-text-muted mb-3 line-clamp-3">
                        {p.brief}
                      </p>
            ...
            >
              <p className="text-sm text-eduverse-text-muted mb-3">
                Built with <span className="text-eduverse-accent">EduVerse</span> — learn to code,
                then prove it.
              </p>
              <Link href="/auth/register" className="nav-cta">
