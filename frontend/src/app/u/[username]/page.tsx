      ...
      <main className="max-w-4xl mx-auto px-5 md:px-8 py-10">
        {state === "loading" && (
          <div className="space-y-6 animate-pulse" aria-hidden="true">
            ...
            <div className="grid sm:grid-cols-2 gap-4">{[1, 2, 3, 4].map((i) => <div key={i} className="h-40 rounded bg-eduverse-surface" />)}</div>
          </div>
        )}
        ...
        {state === "ready" && data && (
          <>
            ...
            <h2 className="flex items-center gap-2 text-sm font-mono text-eduverse-text-muted mt-8 mb-4">
              <span className="text-eduverse-accent">//</span> projects built on EduVerse
            </h2>
            ...
            {data.projects.length === 0 ? (
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {data.projects.map((p, i) => (
                  <motion.div key={p.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.05, duration: 0.45, ease }}>
