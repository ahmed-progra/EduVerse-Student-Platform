...
export default function ProjectWorkspace() {
  ...
  const save = async (nextMilestones?: ProjectMilestone[]) => {
    ...
    setError("");
    try {
      const res = await api.projectUpdate(id as string, {
        code,
  ...
  const submit = async () => {
    ...
    try {
      ...
      api
        .getProfile()
        .then((p) => {
          updateXp(p.data.xp, p.data.level);
          updateCoins(p.data.coins);
        })
        .catch(() => {});
    } catch (err: unknown) {
  ...
  if (offline || !project) {
    return (
      <EmptyState
        icon={offline ? WifiOff : Rocket}
        title={offline ? "Can't reach the server" : "Project not found"}
        message={offline ? "Start the backend and refresh." : "This project doesn't exist."}
      >
        <Link
          href="/projects"
  ...
  return (
    ...
    >
      <motion.div variants={fadeUp} transition={fastEaseTransition}>
        ...
        <Link
          href="/projects"
          className="text-sm text-eduverse-text-muted hover:text-eduverse-accent inline-flex items-center gap-1 mb-3 transition-colors"
        >
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-sm text-eduverse-text-muted">
              <span className="font-mono text-xs">
                {langLabel[project.language] || project.language}
              </span>
              <span className="capitalize">{project.difficulty}</span>
              {done && (
                <span className="flex items-center gap-1 text-eduverse-success">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <motion.div
          variants={fadeUp}
          transition={{ ...fastEaseTransition, delay: 0.05 }}
          className="space-y-4"
        >
          <GlassCard>
            <h2 className="flex items-center gap-2 text-sm font-mono text-eduverse-text-muted mb-2">
              <span className="text-eduverse-accent">//</span> Brief
            </h2>
            ...
            {project.skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {project.skills.map((s) => (
                  <span
                    key={s}
                    className="text-[10px] px-2 py-0.5 rounded-full bg-eduverse-raised text-eduverse-text-muted"
                  >
                    {s}
                  </span>
                ))}
          ...
          {milestones.length > 0 && (
            <GlassCard>
              <h2 className="flex items-center gap-2 text-sm font-mono text-eduverse-text-muted mb-3">
                <span className="text-eduverse-accent">//</span> Milestones
              </h2>
                ...
                {milestones.map((m, i) => (
                  ...
                  >
                    {m.done ? (
                      ...
                      <Circle
                        size={16}
                        className="text-eduverse-text-muted mt-0.5 shrink-0 group-hover:text-eduverse-accent transition-colors"
                        aria-hidden="true"
                      />
                    )}
                    <span
                      className={`text-sm ${m.done ? "text-eduverse-text-muted line-through" : "text-eduverse-text-body"}`}
                    >
                      {m.text}
                    </span>
                  </button>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button
          className="px-4 py-2.5 rounded-[var(--radius-button)] bg-eduverse-accent-strong text-white text-sm font-semibold hover:brightness-110 active:scale-[0.97] transition-[filter,transform] duration-150 disabled:opacity-50 flex items-center gap-2"
          onClick={submit}
          disabled={submitting}
        >
          {submitting ? (
          ...
          <>
            ...
            >
              {project.published ? (
                ...
                <>
                  <Globe size={14} aria-hidden="true" /> Publish to portfolio
                </>
              )}
            </button>
          ...
          <GlassCard>
            <div className="flex items-center gap-4 mb-4">
              <div
                ...
                style={{
                  border: "2px solid var(--color-eduverse-success)",
              ...
              <div>
                <div className="flex items-center gap-2 text-lg font-bold text-eduverse-text font-display">
                  <Award size={18} className="text-eduverse-success" aria-hidden="true" /> Project
                  Reviewed
                </div>
                <span className="flex items-center gap-1 text-sm font-bold font-mono text-eduverse-warning mt-1">
                  <Zap size={13} aria-hidden="true" /> +{project.xpAwarded} XP & coins
                </span>
              </div>
            </div>
                  ...
                  return (
                    <div key={i}>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className="font-mono text-eduverse-text-muted">
                          {r.score}/{r.max}
                        </span>
                      </div>
                      <div className="h-2 rounded-full overflow-hidden bg-eduverse-raised">
                        <div
                          ...
                          style={{ width: `${pct}%`, background: "var(--color-eduverse-accent)" }}
                        />
                      </div>
                      {r.note && (
                        <p className="text-[11px] text-eduverse-text-muted mt-1">{r.note}</p>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="app-card p-3">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-eduverse-text mb-2">
                  <TrendingUp size={13} className="text-eduverse-success" aria-hidden="true" />{" "}
                  Strengths
                </div>
                <ul className="space-y-1">
                  {project.strengths.length ? (
                    project.strengths.map((s, i) => (
                      <li key={i} className="text-xs text-eduverse-text-body">
                        • {s}
                      </li>
                    ))
                  ) : (
              ...
              <div className="app-card p-3">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-eduverse-text mb-2">
                  <AlertCircle size={13} className="text-eduverse-warning" aria-hidden="true" />{" "}
                  Improve next
                </div>
                <ul className="space-y-1">
