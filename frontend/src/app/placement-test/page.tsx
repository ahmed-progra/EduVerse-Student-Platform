...
function PlacementTestContent() {
  ...
  useEffect(() => {
    ...
    api.getCoursePlacementQuestions(courseId).then((res) => {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const res = await api.submitCoursePlacement(courseId!, { answers });
      setResult(res.data);
    } catch {}
    setSubmitting(false);
  };
          ...
          <GlassCard className="text-center p-12 max-w-md" >
            ...
            {result.startingXp > 0 && (
              <div className="flex items-center justify-center gap-2 text-eduverse-warning font-bold mb-6">
                <Zap className="w-5 h-5" /> XP Bonus: {result.startingXp}
              </div>
            )}
