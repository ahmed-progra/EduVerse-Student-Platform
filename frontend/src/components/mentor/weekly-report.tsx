...
import { RefreshCw, ArrowUpRight, ArrowDownRight, AlertCircle, Compass, Hammer } from "lucide-react";
import type { MentorReportData } from "@/lib/mentor-types";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <ReportList title="Improved" icon={<ArrowUpRight size={13} className="text-eduverse-success" />} items={report.improved} tone="success" />
        <ReportList title="Regressed" icon={<ArrowDownRight size={13} className="text-eduverse-danger" />} items={report.regressed} tone="danger" emptyText="Nothing slipped — nice." />
        <ReportList title="Needs work" icon={<AlertCircle size={13} className="text-eduverse-warning" />} items={report.needsWork} tone="warning" emptyText="No gaps flagged." />
      </div>
      ...
      {report.projects.length > 0 && (
        <div>
          <div className="flex items-center gap-2 text-xs font-mono text-eduverse-text-muted mb-2">
            <Hammer size={12} className="text-eduverse-accent" aria-hidden="true" /> Suggested projects
          </div>
              ...
              <div key={i} className="rounded border border-eduverse-border p-3">
                ...
                {p.skills.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {p.skills.map((s) => (
                      <span key={s} className="text-[10px] px-1.5 py-0.5 rounded bg-eduverse-raised text-eduverse-text-muted">
                        {s}
                      </span>
                    ))}
                  </div>
      ...
      <button className="ai-panel-action-btn ai-panel-action-sm ai-panel-action-ghost" onClick={onRefresh}>
