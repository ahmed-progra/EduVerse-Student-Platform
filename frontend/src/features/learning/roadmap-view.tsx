...
import {
  Sprout, Star, Trophy, CheckCircle, Circle, ChevronRight, SkipForward,
  RefreshCw, Clock, Target, TrendingUp, TrendingDown, RotateCcw,
} from "lucide-react";
...
export interface LearningStateData {
  assessed: boolean;
  assessment: { score: number; total: number; level: string; analysis: { summary?: string; scorePct?: number } } | null;
...
export function RoadmapView({
  ...
  const refresh = async () => {
    ...
    try {
      ...
      onStateChange({
        ...state,
        profile: { ...profile, level: res.data.level, strengths: res.data.strengths, weaknesses: res.data.weaknesses },
        roadmap: res.data.roadmap,
        assessment: state.assessment
          ? { ...state.assessment, analysis: { ...state.assessment.analysis, summary: res.data.summary } }
          : state.assessment,
      });
    } catch (err: unknown) {
    ...
    <div className="space-y-5">
      ...
      <GlassCard>
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs font-mono text-eduverse-text-muted">
                  assessment {state.assessment?.analysis?.scorePct ?? Math.round(((state.assessment?.score || 0) / Math.max(1, state.assessment?.total || 1)) * 100)}%
                </span>
              </div>
              <p className="text-xs text-eduverse-text-muted mt-0.5 flex items-center gap-3 flex-wrap">
                <span className="inline-flex items-center gap-1">
                  <Target size={12} aria-hidden="true" /> {required.length} lessons on your path · {skipped.length} skipped
                </span>
                <span className="inline-flex items-center gap-1">
                  <Clock size={12} aria-hidden="true" /> ~{hours > 0 ? `${hours}h ` : ""}{mins}m remaining
                </span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="ai-panel-action-btn ai-panel-action-sm" onClick={refresh} disabled={refreshing} title="AI re-evaluates your profile and rebuilds the path">
              <RefreshCw size={12} className={refreshing ? "animate-spin" : ""} aria-hidden="true" />
              {refreshing ? "Updating…" : "Update my path"}
            </button>
            <button className="ai-panel-action-btn ai-panel-action-sm ai-panel-action-ghost" onClick={onRetake} title="Retake the placement assessment">
        ...
        {state.assessment?.analysis?.summary && (
          <p className="text-sm text-eduverse-text-body mt-4">{state.assessment.analysis.summary}</p>
        )}
        {roadmap.focus && (
          <p className="text-sm mt-2" style={{ color: "var(--color-eduverse-accent)" }}>
            <Target size={13} className="inline mr-1.5" aria-hidden="true" />{roadmap.focus}
          </p>
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {Object.entries(profile.mastery).map(([topic, m]) => (
            <span
              ...
              style={{ border: "1px solid var(--color-eduverse-border)", color: "var(--color-eduverse-text-body)" }}
              title={`${m.status} — ${m.score}/100`}
            >
              <span className="w-2 h-2 rounded-full shrink-0" style={{ background: STATUS_COLOR[m.status] }} aria-hidden="true" />
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <h4 className="text-xs font-semibold text-eduverse-text mb-2 flex items-center gap-1.5">
              <TrendingUp size={14} className="text-eduverse-success" aria-hidden="true" /> Strengths
            </h4>
            {profile.strengths.length ? (
              <ul className="space-y-1">
                {profile.strengths.map((s, i) => (
                  <li key={i} className="text-sm text-eduverse-text-body">· {s}</li>
                ))}
              </ul>
            ) : (
          ...
          <div>
            <h4 className="text-xs font-semibold text-eduverse-text mb-2 flex items-center gap-1.5">
              <TrendingDown size={14} className="text-eduverse-danger" aria-hidden="true" /> Focus areas
            </h4>
      ...
      <div>
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          ...
          {skipped.length > 0 && (
            <button className="text-xs text-eduverse-text-muted hover:text-eduverse-accent transition-colors" onClick={() => setShowSkipped((s) => !s)}>
        ...
        <div className="space-y-2.5">
          {roadmap.items.map((item, i) => {
            ...
            if (item.status === "skipped") {
              return (
                <motion.div key={item.lessonId} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.4), duration: 0.3 }}>
                  <div className="rounded border px-4 py-3" style={{ borderColor: "var(--color-eduverse-border)", opacity: 0.65 }}>
                    <div className="flex items-center gap-3">
                      <SkipForward className="w-4 h-4 text-eduverse-text-muted shrink-0" aria-label="Skipped" />
                      <div className="flex-1 min-w-0">
                        <span className="text-sm text-eduverse-text-muted">
                          <span className="font-mono mr-1.5" style={{ fontSize: "0.75em" }}>{String(item.order).padStart(2, "0")}</span>
                          <s>{item.title}</s>
                        </span>
                        <p className="text-xs mt-0.5" style={{ color: "var(--color-eduverse-text-muted)" }}>
                          <span className="font-semibold" style={{ color: "var(--color-eduverse-success)" }}>Skipped:</span> {item.reason}
                        </p>
                      </div>
                      <Link href={`/lessons/${item.lessonId}`} className="text-xs text-eduverse-text-muted hover:text-eduverse-accent shrink-0 transition-colors">
                        review anyway
                      </Link>
                    </div>
                  </div>
              ...
              <motion.div key={item.lessonId} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.4), duration: 0.3 }}>
                <Link href={`/lessons/${item.lessonId}`} className="block group">
                  <GlassCard
                    ...
                    style={isNext ? { borderColor: "var(--color-eduverse-accent)", boxShadow: "0 0 0 1px var(--color-eduverse-accent)" } : undefined}
                  >
                    {item.completed ? (
                      <CheckCircle className="w-5 h-5 text-eduverse-success shrink-0" aria-label="Completed" />
                    ) : (
                      <Circle className="w-5 h-5 text-eduverse-text-muted shrink-0" aria-hidden="true" />
                    )}
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm truncate" style={{ color: "var(--color-eduverse-text)" }}>
                        <span className="text-eduverse-text-muted font-normal font-mono mr-1.5" style={{ fontSize: "0.8em" }}>
                          {String(item.order).padStart(2, "0")}
                        </span>
                        {item.title}
                        {isNext && (
                          <span className="ml-2 px-1.5 py-0.5 rounded text-[10px] font-mono align-middle"
                            style={{ background: "var(--color-eduverse-accent-soft)", color: "var(--color-eduverse-accent)" }}>
                            START HERE
                          </span>
                        )}
                      </h4>
                    ...
                    <ChevronRight className="w-4 h-4 text-eduverse-text-muted transition-transform duration-200 group-hover:translate-x-1 shrink-0" aria-hidden="true" />
                  </GlassCard>
                </Link>
              </motion.div>
            );
