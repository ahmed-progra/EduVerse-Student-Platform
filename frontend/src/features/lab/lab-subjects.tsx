...
export const LAB_SUBJECTS: LabSubject[] = [
  {
    ...
    blurb: "Mesh real gears, swing a Newton's cradle, stretch a spring, and unleash a chaotic double pendulum — every law of motion made tangible.",
    Icon: Orbit,
    models: [
      {
        id: "spur-gears",
        ...
        controls: [
          { kind: "slider", key: "teeth1", label: "Gear 1 teeth (driver)", min: 8, max: 40, step: 1, default: 20, rebuild: true },
          { kind: "slider", key: "teeth2", label: "Gear 2 teeth (driven)", min: 8, max: 60, step: 1, default: 36, rebuild: true },
          { kind: "slider", key: "rpm", label: "Drive speed (RPM)", min: 10, max: 180, step: 5, default: 60 },
          { kind: "select", key: "dir", label: "Direction", default: "cw", options: [{ label: "Clockwise", value: "cw" }, { label: "Counter-clockwise", value: "ccw" }] },
          { kind: "toggle", key: "animate", label: "Run rotation", default: true },
        ],
        readouts: gearReadouts,
        chart: gearChart,
        toolHints: ["Drag to orbit the gears", "Change the tooth counts", "Watch the gear ratio update"],
        summary: "Two machined spur gears meshing — the driver turns the driven gear at a speed set by their tooth ratio.",
        lesson: [
        ...
        concepts: ["Gear ratio = N₂ / N₁", "Speed vs torque trade-off", "Direction reversal on meshing", "Module & pitch diameter"],
        applications: ["Car gearboxes & transmissions", "Mechanical clocks and watches", "Bicycle drivetrains", "Industrial machinery"],
        tasks: ["Set gear 2 to twice the teeth of gear 1 and read the ratio", "Flip the rotation direction", "Slow the driver to 20 RPM and watch the output"],
        challenge: "Configure a 3:1 reduction, then predict the output RPM before checking the result card.",
      },
      {
        id: "newtons-cradle",
        ...
        controls: [
          ...
          { kind: "slider", key: "pull", label: "Lifted each side", min: 1, max: 3, step: 1, default: 1 },
          { kind: "slider", key: "lift", label: "Swing angle", min: 0.3, max: 1.0, step: 0.05, default: 0.6 },
        ...
        toolHints: ["Drag to orbit the cradle", "Lift two balls instead of one", "Slow the motion down"],
        summary: "A row of polished steel spheres that pass momentum end to end through near-perfect elastic collisions.",
        lesson: [
        ...
        concepts: ["Conservation of momentum", "Conservation of kinetic energy", "Elastic collisions", "Impulse transfer"],
        applications: ["Vehicle crumple-zone design", "Billiards & snooker physics", "Impact & drop testing"],
        tasks: ["Lift two balls and count how many swing out", "Slow the speed to watch the contact", "Add more spheres to the row"],
      ...
      },
      {
        ...
        controls: [
          { kind: "slider", key: "k", label: "Spring constant k (N/m)", min: 5, max: 60, step: 1, default: 20 },
          { kind: "slider", key: "mass", label: "Mass (kg)", min: 0.5, max: 6, step: 0.1, default: 2 },
          { kind: "slider", key: "amp", label: "Amplitude", min: 0.3, max: 2, step: 0.1, default: 1 },
          { kind: "toggle", key: "showForce", label: "Show force vector", default: true },
          { kind: "toggle", key: "animate", label: "Run simulation", default: true },
        ],
        readouts: springReadouts,
        chart: springChart,
        toolHints: ["Drag to orbit", "Double the mass and watch the period", "Toggle the restoring-force arrow"],
        summary: "A helical spring and mass oscillating in simple harmonic motion, with the restoring force drawn live.",
        lesson: [
        ...
        concepts: ["Hooke's law F = −kx", "Period T = 2π√(m/k)", "Restoring force", "Amplitude independence"],
        applications: ["Vehicle suspension systems", "Mechanical watch balance springs", "Seismometers", "Building tuned-mass dampers"],
        tasks: ["Double the mass and watch the period grow", "Stiffen the spring by raising k", "Turn off the force vector, then back on"],
      ...
      },
      {
        ...
        difficulty: "Advanced",
        estMinutes: 8,
        objective: "Witness deterministic chaos — tiny changes in the start angle lead to wildly different motion.",
        controls: [
          { kind: "slider", key: "initAngle", label: "Start angle", min: 0.3, max: 3.0, step: 0.1, default: 1.6, rebuild: true },
          { kind: "slider", key: "len1", label: "Arm 1 length", min: 0.8, max: 2.4, step: 0.1, default: 1.6 },
          { kind: "slider", key: "len2", label: "Arm 2 length", min: 0.8, max: 2.4, step: 0.1, default: 1.6 },
          { kind: "slider", key: "mass1", label: "Mass 1", min: 0.5, max: 3, step: 0.1, default: 1.4 },
          { kind: "slider", key: "mass2", label: "Mass 2", min: 0.5, max: 3, step: 0.1, default: 1.0 },
          { kind: "slider", key: "gravity", label: "Gravity", min: 2, max: 20, step: 0.5, default: 9.8 },
          { kind: "toggle", key: "animate", label: "Run simulation", default: true },
        ],
        readouts: doublePendulumReadouts,
        toolHints: ["Drag the start angle to relaunch", "Watch the golden trace go chaotic", "Make one arm much heavier"],
        summary: "Two linked arms whose combined motion is famously chaotic — the lower bob traces an ever-changing path.",
        lesson: [
        ...
        concepts: ["Deterministic chaos", "Sensitivity to initial conditions", "Coupled oscillators", "Energy conservation"],
        applications: ["Weather & climate modelling", "Robotics & control theory", "Procedural animation physics"],
        tasks: ["Relaunch from a near-vertical start angle", "Make arm 2 heavier than arm 1", "Lower gravity and watch it slow"],
        challenge: "Find a start angle where the trace stays smooth for a few seconds before turning chaotic.",
      },
      {
        id: "orbits",
        ...
        controls: [
          ...
          { kind: "slider", key: "speed", label: "Time speed", min: 0.2, max: 2, step: 0.1, default: 1 },
          { kind: "slider", key: "tilt", label: "Orbit tilt", min: 0, max: 1.6, step: 0.1, default: 1 },
          { kind: "toggle", key: "showRings", label: "Show orbit rings", default: true },
          { kind: "toggle", key: "animate", label: "Run simulation", default: true },
        ],
        readouts: orbitReadouts,
        chart: orbitChart,
        toolHints: ["Drag to view the system edge-on", "Add planets and change the tilt", "Speed up time to compare periods"],
        summary: "A central star lighting up to six planets, each sweeping its orbit at a speed set by its distance.",
        lesson: [
        ...
        concepts: ["Kepler's third law T² ∝ r³", "Orbital radius vs speed", "Gravity as a central force", "Orbital period"],
        applications: ["Satellite & spacecraft orbits", "Planetary science", "Space-mission planning"],
        tasks: ["Add two more planets", "Tilt the orbits to view edge-on", "Speed up time and watch the inner planets lap the outer"],
        challenge: "Estimate how many inner-planet orbits happen during a single outer-planet orbit.",
      },
      {
        id: "pendulum",
        name: "Pendulum wave",
        ...
        estMinutes: 6,
        objective: "Watch graded-length pendulums drift in and out of phase into a travelling wave.",
        controls: [
          { kind: "slider", key: "count", label: "Pendulums", min: 4, max: 16, step: 1, default: 12 },
          { kind: "slider", key: "gravity", label: "Gravity", min: 0.5, max: 2, step: 0.05, default: 1 },
          { kind: "slider", key: "amplitude", label: "Swing angle", min: 0.2, max: 0.9, step: 0.05, default: 0.5 },
          { kind: "toggle", key: "animate", label: "Run simulation", default: true },
        ],
        readouts: pendulumReadouts,
        chart: pendulumChart,
        toolHints: ["Drag to orbit the row", "Add pendulums for a smoother wave", "Raise gravity to speed the swing"],
        summary: "A row of pendulums, each slightly longer than the last, drifting in and out of phase to paint a travelling wave.",
        lesson: [
        ...
        concepts: ["Period depends only on length & g", "Phase drift", "Beat patterns", "Re-synchronisation"],
        applications: ["Wave & oscillation demonstrations", "Metronome arrays", "Teaching resonance"],
        tasks: ["Add pendulums for a smoother wave", "Lower gravity to slow it down", "Watch a full re-synchronisation cycle"],
  ...
  },
  {
    ...
    blurb: "Assemble a working gearbox and trace how power flows — and reduces — through a chain of meshing gears. More mechanisms coming soon.",
    Icon: Wrench,
    models: [
      {
        id: "gearbox",
        ...
        difficulty: "Intermediate",
        estMinutes: 7,
        objective: "Build a multi-stage gear train and see how reductions multiply through the chain.",
        controls: [
          { kind: "slider", key: "t1", label: "Gear 1 teeth (input)", min: 8, max: 30, step: 1, default: 14, rebuild: true },
          { kind: "slider", key: "t2", label: "Gear 2 teeth", min: 8, max: 40, step: 1, default: 26, rebuild: true },
          { kind: "slider", key: "t3", label: "Gear 3 teeth", min: 8, max: 30, step: 1, default: 16, rebuild: true },
          { kind: "slider", key: "t4", label: "Gear 4 teeth (output)", min: 8, max: 48, step: 1, default: 32, rebuild: true },
          { kind: "slider", key: "rpm", label: "Input speed (RPM)", min: 20, max: 180, step: 5, default: 90 },
          { kind: "toggle", key: "animate", label: "Run gearbox", default: true },
        ],
        readouts: gearTrainReadouts,
        chart: gearTrainChart,
        toolHints: ["Drag to orbit the gearbox", "Resize any gear in the chain", "Watch the output speed drop"],
        summary: "Four meshing spur gears forming a gear train — power enters at one end and leaves slower but stronger at the other.",
        lesson: [
        ...
        concepts: ["Compound gear trains", "Idler gears & direction", "Overall reduction = N_out / N_in", "Torque multiplication"],
        applications: ["Industrial gearboxes", "Robotic joints & actuators", "Wind-turbine drivetrains", "Conveyor systems"],
        tasks: ["Make the output gear the largest in the chain", "Match the input and output gear sizes", "Push the input to 180 RPM"],
        challenge: "Configure the train for the slowest possible output, then read the overall ratio.",
      },
    ],
  },
  {
    ...
    blurb: "Drive multivariable surfaces and parametric curves in real time. Change the function, the amplitude, the frequencies — and watch the geometry respond.",
    Icon: Sigma,
    models: [
      {
        id: "surface",
        ...
        difficulty: "Intermediate",
        estMinutes: 8,
        objective: "See how a function of two variables becomes a 3D landscape with peaks, valleys, and saddles.",
        controls: [
          {
            kind: "select",
            key: "fn",
          ...
          { kind: "slider", key: "amp", label: "Amplitude", min: 0.3, max: 1.8, step: 0.05, default: 1 },
          { kind: "slider", key: "freq", label: "Frequency", min: 0.3, max: 1.8, step: 0.05, default: 1 },
        ...
        chart: mathChart,
        toolHints: ["Drag to orbit the surface", "Scroll to zoom in and out", "Switch the function and amplitude"],
        summary: "The graph of a function of two variables, colour-mapped by height and rippling through time.",
        lesson: [
        ...
        concepts: ["Functions of two variables z = f(x,y)", "Maxima, minima & saddle points", "Amplitude vs frequency", "Level sets & gradients"],
        applications: ["Terrain & height-map generation", "Optimization landscapes in ML", "Wave interference in physics"],
        tasks: ["Switch to the saddle and find the saddle point", "Raise the frequency until the ripples double", "Toggle the wireframe to reveal the mesh"],
        challenge: "Find a function and amplitude where the peak height is about double the deepest valley.",
      },
      {
        id: "lissajous",
        ...
        controls: [
          { kind: "slider", key: "a", label: "X frequency", min: 1, max: 7, step: 1, default: 3, rebuild: true },
          { kind: "slider", key: "b", label: "Y frequency", min: 1, max: 7, step: 1, default: 2, rebuild: true },
          { kind: "slider", key: "c", label: "Z frequency", min: 1, max: 7, step: 1, default: 4, rebuild: true },
          { kind: "toggle", key: "animate", label: "Trace the curve", default: true },
        ],
        readouts: lissajousReadouts,
        chart: lissajousChart,
        toolHints: ["Drag to orbit the knot", "Change the frequency ratios", "Watch the bead trace the path"],
        summary: "A single point moving as three independent sine waves — one per axis — sweeps out a 3D Lissajous figure.",
        lesson: [
        ...
        concepts: ["Parametric equations", "Frequency ratios", "Closed vs open curves", "Phase relationships"],
        applications: ["Oscilloscope & laser art", "Harmonographs", "Signal & vibration analysis"],
        tasks: ["Make a 1 : 1 : 1 figure", "Find a ratio that does not close", "Watch the bead complete one full loop"],
      ...
      },
      {
        ...
        difficulty: "Intermediate",
        estMinutes: 7,
        objective: "See how a 2×2 matrix bends space — stretching, rotating, shearing and flipping the grid.",
        controls: [
        ...
        chart: matrixChart,
        toolHints: ["Move a slider and watch the grid bend", "Make the determinant negative to flip space", "Set b and c to 0 for a pure scale"],
        summary: "The plane warped by a live 2×2 matrix — the amber and indigo arrows are where the basis vectors î and ĵ land.",
        lesson: [
        ...
        concepts: ["Linear transformations", "Basis vectors î and ĵ", "Determinant as area scale", "Shear, rotation & reflection"],
        applications: ["Computer graphics & game engines", "Robotics & kinematics", "Data transforms in ML", "Image warping"],
        tasks: ["Build a 90° rotation (a=0, c=1, b=−1, d=0)", "Make a shear with b = 1", "Drive the determinant negative and watch it flip"],
        challenge: "Find a matrix whose determinant is exactly 0, and explain what happens to the grid.",
      },
      {
        id: "calculus",
        ...
        difficulty: "Advanced",
        estMinutes: 8,
        objective: "Drag a point along a curve to see its slope (the derivative) and the area beneath it (the integral).",
        controls: [
          {
            kind: "select",
            key: "fn",
        ...
        chart: calculusChart,
        toolHints: ["Drag x₀ and watch the slope change", "Switch to Integral to shade the area", "Try the sine and cubic curves"],
        summary: "A curve with a blue point you can slide: its violet tangent shows the derivative, and the amber bars fill in the integral.",
        lesson: [
        ...
        concepts: ["Derivative as instantaneous slope", "Tangent lines", "Definite integral as area", "Fundamental Theorem of Calculus"],
        applications: ["Velocity & acceleration in physics", "Marginal cost in economics", "Gradient descent in ML", "Areas, volumes & probability"],
        tasks: ["Find where the parabola's slope is 0", "Shade the area from −3 to 0 on the sine", "Compare the cubic's slope at x = −1 and x = 1"],
        challenge: "On the sine curve, find an x₀ where the integral from −3 is back to (near) zero, and explain why.",
      },
    ],
  },
  {
    ...
    blurb: "Explore a real uploaded 3D cell alongside a built-in DNA helix you can re-wind. Drag to orbit, scroll to zoom, peel the model apart.",
    Icon: FlaskConical,
    models: [
      {
        id: "animal-cell",
        ...
        controls: [
          { kind: "slider", key: "explode", label: "Explode view", min: 0, max: 1.6, step: 0.05, default: 0 },
          { kind: "toggle", key: "wireframe", label: "Wireframe", default: false },
        ],
        readouts: (p) => [
          { label: "Cell type", value: "Eukaryotic" },
          { label: "Format", value: "glTF 2.0 (GLB)" },
          { label: "Explode", value: `${(typeof p.explode === "number" ? p.explode : 0).toFixed(2)}×` },
          { label: "Shading", value: p.wireframe ? "wireframe" : "PBR" },
        ],
        toolHints: ["Drag to orbit the cell", "Scroll to zoom into organelles", "Pull the Explode slider to separate parts"],
        summary: "A real eukaryotic animal cell, sliced open so you can explore its organelles from every angle.",
        lesson: [
        ...
        concepts: ["Eukaryotic vs prokaryotic cells", "Membrane-bound organelles", "Nucleus, mitochondria, ER, Golgi", "Selective permeability"],
        applications: ["Medicine & disease research", "Biotechnology", "Genetics & cell biology"],
        tasks: ["Explode the cell to separate the organelles", "Find the nucleus", "Zoom into a mitochondrion"],
      ...
      },
      {
        ...
        difficulty: "Intermediate",
        estMinutes: 6,
        objective: "Understand the double-helix structure and where proteins read the genetic code.",
        controls: [
          { kind: "slider", key: "turns", label: "Turns", min: 2, max: 5, step: 1, default: 3, rebuild: true },
          { kind: "slider", key: "radius", label: "Helix radius", min: 1.1, max: 2.4, step: 0.05, default: 1.7 },
          { kind: "slider", key: "twist", label: "Extra twist", min: 0, max: 1, step: 0.05, default: 0 },
          { kind: "toggle", key: "showRungs", label: "Show base pairs", default: true },
        ],
        readouts: dnaReadouts,
        toolHints: ["Drag to spin the helix", "Widen the radius or add turns", "Toggle the base-pair rungs"],
        summary: "Two sugar-phosphate backbones winding around a shared axis, joined by colour-coded base-pair rungs.",
        lesson: [
        ...
        concepts: ["The double helix", "Sugar-phosphate backbones", "Complementary base pairs", "Major & minor grooves"],
        applications: ["Genetics & heredity", "DNA sequencing", "Forensic science", "Gene therapy"],
        tasks: ["Add more turns to the helix", "Widen the helix radius", "Hide the base pairs to study the backbones"],
      ...
      },
      {
        ...
        difficulty: "Intermediate",
        estMinutes: 7,
        objective: "Trace a nerve impulse from the dendrites, down the myelinated axon, to the terminals.",
        controls: [
          { kind: "slider", key: "dendrites", label: "Dendrites", min: 3, max: 8, step: 1, default: 5, rebuild: true },
          { kind: "slider", key: "myelin", label: "Myelin segments", min: 3, max: 8, step: 1, default: 5, rebuild: true },
          { kind: "toggle", key: "animate", label: "Fire the neuron", default: true },
        ],
        readouts: neuronReadouts,
        chart: neuronChart,
        toolHints: ["Watch the blue pulse race down the axon", "Add dendrites to gather more input", "More myelin = faster signalling"],
        summary: "A single neuron: a soma with branching dendrites, a long myelin-sheathed axon, and terminals that pass the signal on.",
        lesson: [
        ...
        concepts: ["Neuron structure", "Action potentials", "Threshold & all-or-nothing firing", "Myelin & saltatory conduction"],
        applications: ["Neuroscience & medicine", "Brain–computer interfaces", "Neural-network inspiration", "Understanding MS & nerve disease"],
        tasks: ["Fire the neuron and follow the pulse", "Add dendrites to gather more input", "Read the voltage spike on the chart"],
  ...
  },
  {
    ...
    blurb: "Orbit real molecular geometries — bent water, tetrahedral methane, a benzene ring — in ball-and-stick or space-filling form, and read their composition live.",
    Icon: Atom,
    models: [
      {
        id: "molecules",
        ...
        chart: moleculeChart,
        toolHints: ["Drag to orbit the molecule", "Switch molecules to compare shapes", "Try the space-filling view"],
        summary: "Real molecular geometries rendered atom by atom — oxygen in red, carbon in grey, nitrogen in blue, hydrogen in white.",
        lesson: [
        ...
        concepts: ["Covalent bonding", "Molecular geometry (VSEPR)", "Bond angles", "Ball-and-stick vs space-filling"],
        applications: ["Drug design & pharmacology", "Materials science", "Biochemistry", "Environmental chemistry"],
        tasks: ["Compare water's bent shape to linear CO₂", "Count the atoms in benzene", "Switch methane to space-filling"],
        challenge: "Explain why CO₂ is linear but water is bent, even though both have a central atom with two bonds.",
      },
    ],
  },
  {
    ...
    blurb: "Watch a shuffled array sort itself bar by bar, race the classic algorithms, and see why O(n²) and O(n log n) are worlds apart.",
    Icon: Cpu,
    models: [
      {
        id: "sorting",
        ...
        difficulty: "Intermediate",
        estMinutes: 7,
        objective: "Compare how sorting algorithms order data — and feel the gap between O(n²) and O(n log n).",
        controls: [
          {
            kind: "select",
            key: "algo",
          ...
          { kind: "slider", key: "size", label: "Array size (n)", min: 6, max: 40, step: 1, default: 16, rebuild: true },
          { kind: "slider", key: "speed", label: "Speed", min: 0.5, max: 4, step: 0.1, default: 1.5 },
          { kind: "toggle", key: "animate", label: "Run", default: true },
        ],
        readouts: sortingReadouts,
        chart: sortingChart,
        toolHints: ["Switch the algorithm and re-race", "Grow the array and watch the cost climb", "Slow it down to follow each compare"],
        summary: "A shuffled bar chart sorting itself in real time — amber bars are being compared, green bars are locked into their final place.",
        lesson: [
        ...
        concepts: ["Comparisons & swaps", "In-place sorting", "Big-O time complexity", "Why O(n log n) beats O(n²)"],
        applications: ["Databases & query engines", "Search-result ranking", "Graphics & spatial indexing", "Any 'order this list' task"],
        tasks: ["Race bubble vs quicksort at n = 40", "Slow the speed and follow one pass", "Watch the green sorted region grow"],
