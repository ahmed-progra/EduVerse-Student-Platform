      ...
      {items.length === 0 ? (
        ...
        <>
          <div className="flex flex-wrap gap-1.5">
            {items.map((t) => {
              const href = links?.[t];
              const className = "text-xs px-2.5 py-1 rounded-full border inline-flex items-center gap-1 transition-colors";
              const style = { borderColor: color, color: "var(--color-eduverse-text-body)", background: "var(--color-eduverse-raised)" };
              return href ? (
                <Link key={t} href={href} className={`${className} hover:border-eduverse-accent`} style={style} title={`Teach ${t} to Pip`}>
                  {t}
                  <Sprout size={11} className="text-eduverse-accent" aria-hidden="true" />
                </Link>
              ) : (
                <span key={t} className={className} style={style}>{t}</span>
              );
            })}
          </div>
          {hint && links && Object.keys(links).length > 0 && <p className="text-[11px] text-eduverse-text-muted mt-2.5">{hint}</p>}
        </>
      )}
    </div>
  );
